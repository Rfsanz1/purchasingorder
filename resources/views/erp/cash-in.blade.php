            ...
            <div class="p-6">
                ...
                <form @submit.prevent="save()" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Sumber Dana</label><input x-model="form.akun_kas" type="text" placeholder="Bank, cash, dll" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"></div>
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Metode Bayar</label>
                            <select x-model="form.metode_pembayaran" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"><option>Transfer</option><option>Cash</option><option>Giro</option><option>QRIS</option></select>
                        </div>
                    </div>
...
<script>
function cashInApp(){return{
items:[],stats:{},loading:false,saving:false,search:'',filterKat:'',filterDate:'',page:1,total:0,showModal:false,editMode:false,errMsg:'',
toast:{show:false,msg:'',type:'success'},
form:{tanggal:new Date().toISOString().slice(0,10),jumlah:'',keterangan:'',kategori:'Penjualan',akun_kas:'',referensi:'',metode_pembayaran:'Transfer'},
async init(){await this.load()},
async load(){
    this.loading=true;
    try{
        const p=new URLSearchParams({jenis:'masuk',search:this.search,kategori:this.filterKat,bulan:this.filterDate,page:this.page});
...
openAdd(){this.editMode=false;this.errMsg='';this.form={tanggal:new Date().toISOString().slice(0,10),jumlah:'',keterangan:'',kategori:'Penjualan',akun_kas:'',referensi:'',metode_pembayaran:'Transfer'};this.showModal=true},
...
async save(){
    ...
    try{
        const payload={...this.form,jenis:'masuk'};
        ...
        if(r.ok){
            ...
            this.showToast(this.editMode?'Data diupdate':'Kas masuk dicatat','success');
