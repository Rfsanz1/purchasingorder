...
<div x-data="expenseApp()" x-init="init()" class="p-4 md:p-6 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border p-4 shadow-sm"><p class="text-xs text-gray-500">Bulan Ini</p><p class="text-xl font-bold text-orange-600" x-text="formatRp(items.filter(i=>(i.tanggal||'').startsWith(bulanIni)).reduce((a,i)=>a+(parseFloat(i.jumlah)||0),0))"></p></div>
        <div class="bg-white rounded-xl border p-4 shadow-sm"><p class="text-xs text-gray-500">Transaksi Bulan Ini</p><p class="text-2xl font-bold text-blue-600" x-text="items.filter(i=>(i.tanggal||'').startsWith(bulanIni)).length"></p></div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b"><tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Metode</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Jumlah</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Aksi</th>
                </tr></thead>
                <tbody class="divide-y divide-gray-50">
                    <template x-if="loading"><tr><td colspan="6" class="px-4 py-12 text-center text-gray-400">Memuat...</td></tr></template>
                    <template x-if="!loading&&filtered.length===0"><tr><td colspan="6" class="px-4 py-12 text-center text-gray-400"><p class="font-medium">Belum ada pengeluaran tercatat</p><button @click="openAdd()" class="mt-3 bg-red-600 text-white px-4 py-2 rounded-lg text-sm">+ Catat Pengeluaran</button></td></tr></template>
                    <template x-for="tx in filtered" :key="tx.id">
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-gray-500 text-xs" x-text="fmt(tx.tanggal)"></td>
                            <td class="px-4 py-3 font-medium text-gray-900" x-text="tx.deskripsi||'-'"></td>
                            ...
                            <td class="px-4 py-3 hidden md:table-cell text-gray-500 text-xs" x-text="tx.metode_bayar||'-'"></td>
            ...
            <div class="p-6">
                ...
                <form @submit.prevent="save()" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1">Keterangan / Deskripsi *</label><input x-model="form.deskripsi" type="text" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Kategori *</label><select x-model="form.kategori" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"><option>Transportasi</option><option>Makan</option><option>ATK</option><option>Utilitas</option><option>Maintenance</option><option>Marketing</option><option>Lainnya</option></select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Metode Bayar</label><select x-model="form.metode_bayar" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"><option value="">- Pilih -</option><option>Cash</option><option>Transfer</option><option>Kartu Kredit</option><option>QRIS</option></select></div>
                    </div>
...
<script>
function expenseApp(){return{
items:[],filtered:[],loading:false,saving:false,search:'',filterKat:'',filterBulan:'',showModal:false,editMode:false,errMsg:'',
bulanIni:new Date().toISOString().slice(0,7),
toast:{show:false,msg:'',type:'success'},
form:{tanggal:new Date().toISOString().slice(0,10),jumlah:'',deskripsi:'',kategori:'Lainnya',metode_bayar:''},
async init(){await this.load()},
async load(){
    this.loading=true;
    try{
        const r=await fetch('/api/erp/expenses');
        if(r.ok){const d=await r.json();this.items=d.data||[]}
        else this.items=[];
    }catch{this.items=[]}
...
filter(){
    let a=this.items;
    if(this.search)a=a.filter(i=>(i.deskripsi||'').toLowerCase().includes(this.search.toLowerCase())||(i.kategori||'').toLowerCase().includes(this.search.toLowerCase()));
    if(this.filterKat)a=a.filter(i=>i.kategori===this.filterKat);
    if(this.filterBulan)a=a.filter(i=>(i.tanggal||'').startsWith(this.filterBulan));
    this.filtered=a;
},
openAdd(){this.editMode=false;this.errMsg='';this.form={tanggal:new Date().toISOString().slice(0,10),jumlah:'',deskripsi:'',kategori:'Lainnya',metode_bayar:''};this.showModal=true},
editItem(tx){this.editMode=true;this.errMsg='';this.form={...tx};this.showModal=true},
delItem(tx){
    if(!confirm('Hapus pengeluaran ini?'))return;
    fetch(`/api/erp/expenses/${tx.id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]')?.content||''}})
        .then(r=>{if(r.ok){this.items=this.items.filter(i=>i.id!==tx.id);this.filter();this.showToast('Pengeluaran dihapus','success')}});
},
async save(){
    this.saving=true;this.errMsg='';
    try{
        const url=this.editMode?`/api/erp/expenses/${this.form.id}`:'/api/erp/expenses';
        ...
        if(r.ok){
            ...
            else this.items.unshift(d.data||{id:d.id||Date.now(),...this.form});
            this.filter();
            this.showToast('Pengeluaran disimpan','success');
