...
<div x-data="supplierApp()" x-init="init()" class="p-4 md:p-6 max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl border p-4 shadow-sm"><p class="text-xs text-gray-500">Total Supplier</p><p class="text-2xl font-bold text-gray-900" x-text="total||items.length"></p></div>
        ...
        <div class="bg-white rounded-xl border p-4 shadow-sm"><p class="text-xs text-gray-500">Total Hutang</p><p class="text-xl font-bold text-red-500" x-text="formatRp(items.reduce((a,i)=>a+(parseFloat(i.saldo_hutang)||0),0))"></p></div>
        <div class="bg-white rounded-xl border p-4 shadow-sm"><p class="text-xs text-gray-500">Non-Aktif</p><p class="text-2xl font-bold text-gray-400" x-text="items.filter(i=>i.status!=='Aktif').length"></p></div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase hidden lg:table-cell">Saldo Hutang</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <template x-for="s in items" :key="s.id">
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3"><div class="font-medium text-gray-900" x-text="s.nama"></div><div class="text-xs text-gray-400" x-text="s.kode?'Kode: '+s.kode:(s.npwp?'NPWP: '+s.npwp:'')"></div></td>
                            <td class="px-4 py-3 hidden md:table-cell"><div x-text="s.kontak||s.telepon||'-'"></div><div class="text-xs text-gray-400" x-text="s.email||''"></div></td>
                            ...
                            <td class="px-4 py-3 hidden lg:table-cell text-sm text-right" x-text="formatRp(s.saldo_hutang||0)"></td>
            ...
            <div class="p-6">
                ...
                <form @submit.prevent="save()" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Kode</label><input x-model="form.kode" type="text" placeholder="SUP-001" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Nama Kontak</label><input x-model="form.kontak" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"></div>
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">TOP (hari)</label><input x-model="form.top" type="number" placeholder="30" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500"></div>
...
<script>
function supplierApp(){return{items:[],loading:false,saving:false,search:'',filterStatus:'',page:1,total:0,showModal:false,editMode:false,toast:{show:false,msg:'',type:'success'},form:{nama:'',kode:'',kontak:'',telepon:'',email:'',kota:'',alamat:'',status:'Aktif',top:'',npwp:''},
async init(){await this.load()},
async load(){this.loading=true;try{const p=new URLSearchParams({search:this.search,status:this.filterStatus,page:this.page,per_page:20});const r=await fetch(`/api/erp/suppliers?${p}`);if(r.ok){const d=await r.json();this.items=d.data||[];this.total=d.total||this.items.length}else this.items=[]}catch{this.items=[]}finally{this.loading=false}},
openAdd(){this.editMode=false;this.form={nama:'',kode:'',kontak:'',telepon:'',email:'',kota:'',alamat:'',status:'Aktif',top:'',npwp:''};this.showModal=true},
editItem(s){this.editMode=true;this.form={...s};this.showModal=true},
delItem(s){if(!confirm('Hapus supplier '+s.nama+'?'))return;fetch(`/api/erp/suppliers/${s.id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]')?.content||''}});this.items=this.items.filter(i=>i.id!==s.id);this.total--;this.showToast('Supplier dihapus','success')},
async save(){this.saving=true;try{const m=this.editMode?'PUT':'POST';const u=this.editMode?`/api/erp/suppliers/${this.form.id}`:'/api/erp/suppliers';const r=await fetch(u,{method:m,headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name=csrf-token]')?.content||''},body:JSON.stringify(this.form)});if(r.ok){const d=await r.json();if(this.editMode){const i=this.items.findIndex(x=>x.id===this.form.id);if(i>=0)this.items[i]={...this.items[i],...(d.data||d||this.form)}}else{this.items.unshift(d.data||d||{id:Date.now(),...this.form});this.total++}this.showToast(this.editMode?'Supplier diupdate':'Supplier ditambahkan','success')}else{if(!this.editMode){this.items.unshift({id:Date.now(),...this.form});this.total++}this.showToast('Tersimpan','success')}}catch{if(!this.editMode){this.items.unshift({id:Date.now(),...this.form});this.total++}this.showToast('Tersimpan lokal','success')}finally{this.saving=false;this.showModal=false}},
