...
router.post("/auth/login", (req, res): void => {
  ...
  if (!role) {
    res.status(400).json({ ok: false, error: "Role wajib diisi" });
    return;
  }
  ...
  // Driver: cukup pilih nama, tidak perlu password.
  if (role === "driver") {
    const name = (username ?? "").trim().toLowerCase();
    const known = ["yanto", "wawan", "chaidar"];
  ...
  if (!password) {
    res.status(400).json({ ok: false, error: "Password wajib diisi" });
    return;
  }
