...
@section('content')
<div x-data="adminApp()" x-init="init()" class="min-h-screen bg-gray-50">
    ...
    <div class="bg-gradient-to-r from-blue-900 to-blue-700 text-white px-4 pt-8 pb-6">
        <div class="max-w-3xl mx-auto">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-200 text-sm" x-text="isSales ? 'Sales Dashboard' : 'Admin Dashboard'"></p>
                    <h1 class="text-xl font-bold" x-text="isSales ? ('Sales: ' + salesUsername) : 'Super Admin'"></h1>
                </div>
                <div class="flex gap-2">
                    <button @click="runHealthCheck()" class="bg-white/20 hover:bg-white/30 text-white text-xs px-3 py-1.5 rounded-lg">
                        Health Check
                    </button>
                    <a href="/" class="bg-white/20 hover:bg-white/30 text-white text-xs px-3 py-1.5 rounded-lg">Keluar</a>
                </div>
            </div>
            {{-- Stats --}}
            <div class="grid grid-cols-4 gap-3 mt-5">
                <div class="bg-white/10 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold" x-text="filteredOrders.length"></p>
                    <p class="text-blue-200 text-xs">Total</p>
                </div>
                <div class="bg-white/10 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold" x-text="filteredOrders.filter(o=>o.statusPengiriman==='Menunggu').length"></p>
                    <p class="text-blue-200 text-xs">Menunggu</p>
                </div>
                <div class="bg-white/10 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold" x-text="filteredOrders.filter(o=>o.statusPengiriman==='Dikirim').length"></p>
                    <p class="text-blue-200 text-xs">Dikirim</p>
                </div>
                <div class="bg-white/10 rounded-xl p-3 text-center">
                    <p class="text-xl font-bold" x-text="filteredOrders.filter(o=>o.statusPengiriman==='Selesai').length"></p>
    ...
    <div class="max-w-3xl mx-auto px-4">
        <div class="flex gap-1 bg-white rounded-xl shadow-sm mt-4 p-1 overflow-x-auto scrollbar-hide">
    ...
    {{-- PESANAN TAB --}}
    <div x-show="tab==='pesanan'" class="max-w-3xl mx-auto px-4 mt-4 pb-10">
        {{-- Filters --}}
    ...
    {{-- PENGIRIMAN TAB --}}
    <div x-show="tab==='pengiriman'" class="max-w-3xl mx-auto px-4 mt-4 pb-10">
        <div class="space-y-3">
            <template x-if="loading">
                <div class="bg-white rounded-2xl p-8 text-center shadow-sm">
                    <div class="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-2"></div>
    ...
    {{-- WILAYAH TAB --}}
    <div x-show="tab==='wilayah'" class="max-w-3xl mx-auto px-4 mt-4 pb-10">
    ...
    {{-- PENGATURAN TAB --}}
    <div x-show="tab==='pengaturan'" class="max-w-3xl mx-auto px-4 mt-4 pb-10">
        <div class="bg-white rounded-2xl shadow-sm p-5">
