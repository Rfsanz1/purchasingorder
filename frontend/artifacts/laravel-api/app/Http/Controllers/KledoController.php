...
class KledoController extends Controller
{
    ...
    public function products(Request $request): JsonResponse
    {
        ...
        try {
            $url  = "{$this->kledoBase}/products?per_page=20&page={$page}&search=" . urlencode($search);
            $resp = $this->httpGet($url);
            $data = json_decode($resp['body'], true);

            if (!($data['success'] ?? false)) {
                return response()->json(['error' => 'Gagal mengambil produk dari Kledo'], 502);
            }
            ...
            $this->cache[$cacheKey] = ['data' => $result, 'exp' => time() + 300];
            return response()->json($result);
        } catch (\Exception $e) {
            \Log::error('Kledo products fetch error: ' . $e->getMessage());
            return response()->json(['error' => 'Koneksi ke Kledo gagal'], 500);
        }
    }
