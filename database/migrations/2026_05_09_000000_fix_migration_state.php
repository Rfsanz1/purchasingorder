...
/**
 * Fix migration yang stuck/error
 * Migration ini memastikan database state konsisten
 */
return new class extends Migration
{
    public function up(): void
    {
        // Jika sudah di-run, skip
        if ($this->isMigrationAlreadyRun()) {
            return;
        }

        // Pastikan tabel orders ada dengan struktur yang benar
    ...
    {
        // Don't drop, just skip
    }

    /**
     * Cek apakah migration sudah di-run
     */
    private function isMigrationAlreadyRun(): bool
    {
        try {
            return DB::table('migrations')
                ->where('migration', '2026_05_09_000000_fix_migration_state')
                ->exists();
        } catch (\Exception $e) {
            return false;
        }
