...
export async function productsToExcelBuffer(products) {
    ...
    for (const p of products)
        ws.addRow({ id: p.id, sku: p.sku, name: p.name, price: Number(p.hargaJual ?? p.price ?? 0), stock: p.stok ?? p.stock ?? 0, unit: p.unit?.name ?? p.unit ?? '' });
    const buf = await wb.xlsx.writeBuffer();
    return Buffer.from(buf);
}
export async function stockOpnameToExcelBuffer(opname) {
    const wb = new ExcelJS.Workbook();
...
export async function excelBufferToStockOpnameItems(buffer) {
    const wb = new ExcelJS.Workbook();
...
export async function excelBufferToProducts(buffer) {
    const wb = new ExcelJS.Workbook();
    await wb.xlsx.load(buffer);
