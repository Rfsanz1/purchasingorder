...
let InvoiceService = class InvoiceService {
    ...
    async delete(id) {
        ...
        if (!inv)
            throw new NotFoundException('Invoice tidak ditemukan');
        if (inv.status !== 'draft')
            throw new BadRequestException('Hanya invoice draft yang dapat dihapus');
    ...
    async addPayment(invoiceId, dto) {
        if (!dto.amount || dto.amount <= 0)
            throw new BadRequestException('Jumlah pembayaran harus diisi');
    ...
    async issueCreditNote(invoiceId, dto) {
        if (!dto.amount || dto.amount <= 0)
            throw new BadRequestException('Jumlah credit note harus diisi');
    ...
    async sendReminder(id, dto) {
        return this.sendWhatsApp(id, dto ?? {});
    }
    async setRecurring(id, dto) {
        await this.findOne(id);
    ...
    async createPaymentLink(id, dto) {
        await this.findOne(id);
    ...
    async getPaymentByToken(token) {
        ...
        if (!link)
            throw new NotFoundException('Payment link tidak ditemukan');
    ...
    async getPdfHtml(id) {
      ...
      <tbody>${(inv.items ?? []).map((it, i) => `<tr><td>${i + 1}</td><td>${it.nama}</td><td>${it.qty}</td><td>Rp ${Number(it.harga).toLocaleString('id-ID')}</td><td>Rp ${Number(it.subtotal).toLocaleString('id-ID')}</td></tr>`).join('')}</tbody></table>
      <div class="totals">
