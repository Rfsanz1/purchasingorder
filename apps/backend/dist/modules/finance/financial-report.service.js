...
let FinancialReportService = class FinancialReportService {
    ...
    async getBalancesUpTo(dateTo) {
        const accounts = await this.prisma.account.findMany({ where: { isActive: true } });
        const where = { journal: { status: 'POSTED' } };
        if (dateTo)
            where.journal.tanggal = { lte: new Date(dateTo) };
        return this._buildMap(accounts, where);
    }
    async getBalancesInPeriod(dateFrom, dateTo) {
        const accounts = await this.prisma.account.findMany({ where: { isActive: true } });
        const where = { journal: { status: 'POSTED' } };
        const tanggal = {};
        if (dateFrom) {
        ...
        if (Object.keys(tanggal).length > 0)
            where.journal.tanggal = tanggal;
        return this._buildMap(accounts, where);
    }
    async _buildMap(accounts, where) {
        const map = new Map();
        const results = await Promise.all(accounts.map((acc) => this.prisma.journalLine
    ...
    async getIncomeStatement(dateFrom, dateTo, compare = false) {
    ...
    async getStatementOfEquity(dateFrom, dateTo) {
    ...
    async getExecutiveSummary(dateFrom, dateTo) {
        const balanceSheet = await this.getBalanceSheet(dateTo);
        const incomeStatement = await this.getIncomeStatement(dateFrom, dateTo);
    ...
    async exportAsExcel(reportType, payload) {
        ...
        if (reportType === 'Balance Sheet') {
            ...
            sheet.addRow([]);
            sheet.addRow(['Assets']);
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Balance'], ...payload.assets.items.map((item) => [item.code, item.name, item.balance])]);
            sheet.addRow(['Total Assets', payload.assets.total]);
            sheet.addRow([]);
            sheet.addRow(['Liabilities']);
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Balance'], ...payload.liabilities.items.map((item) => [item.code, item.name, item.balance])]);
            sheet.addRow(['Total Liabilities', payload.liabilities.total]);
            sheet.addRow([]);
            sheet.addRow(['Equity']);
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Balance'], ...payload.equity.items.map((item) => [item.code, item.name, item.balance])]);
            ...
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Amount'], ...payload.revenues.items.map((item) => [item.code, item.name, item.balance])]);
            sheet.addRow(['Total Revenue', payload.revenues.total]);
            sheet.addRow([]);
            sheet.addRow(['HPP']);
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Amount'], ...payload.hpp.items.map((item) => [item.code, item.name, item.balance])]);
            sheet.addRow(['Total HPP', payload.hpp.total]);
            sheet.addRow([]);
            sheet.addRow(['Operational Expenses']);
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Amount'], ...payload.operationalExpenses.items.map((item) => [item.code, item.name, item.balance])]);
            ...
            this.renderTableToSheet(sheet, [['Code', 'Name', 'Amount'], ...payload.equity.items.map((item) => [item.code, item.name, item.balance])]);
            sheet.addRow([]);
    ...
    async exportAsPdf(reportType, payload) {
        ...
        if (reportType === 'Balance Sheet') {
            ...
            const section = (label, items) => {
                ...
                items.forEach((item) => doc.fontSize(10).text(`- ${item.code} ${item.name}: ${item.balance}`));
                doc.moveDown();
            };
            ...
            doc.fontSize(12).text(`Period: ${payload.period.dateFrom} - ${payload.period.dateTo}`);
            doc.moveDown();
            payload.equity.items.forEach((item) => doc.fontSize(10).text(`- ${item.code} ${item.name}: ${item.balance}`));
            doc.moveDown();
    ...
    async exportReport(reportType, format, date, dateFrom, dateTo, compare = false) {
        ...
        let payload;
        if (typeName === 'Balance Sheet')
            payload = await this.getBalanceSheet(date);
        else if (typeName === 'Income Statement')
            payload = await this.getIncomeStatement(dateFrom, dateTo, compare);
        else if (typeName === 'Cash Flow')
            payload = await this.getCashFlow(dateFrom, dateTo);
        else if (typeName === 'Statement of Equity')
            payload = await this.getStatementOfEquity(dateFrom, dateTo);
        else if (typeName === 'Executive Summary')
            payload = await this.getExecutiveSummary(dateFrom, dateTo);
        else
            payload = await this.getBalanceSheet(date);
    ...
    async getCashFlow(dateFrom, dateTo) {
        ...
        const [deltaReceivables, deltaInventory, deltaPayables, deltaFixedAssets, deltaLoans] = await Promise.all([
            sumByPrefix('12'),
            sumByPrefix('14'),
            sumByPrefix('21'),
            sumByPrefix('15'),
        ...
        const operatingCF = income.netIncome - deltaReceivables - deltaInventory + deltaPayables;
