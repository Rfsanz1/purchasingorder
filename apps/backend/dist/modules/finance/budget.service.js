...
let BudgetService = class BudgetService {
    ...
    async updateBudget(id, dto) {
        ...
        if (lines) {
            ...
            await this.prisma.budgetLine.createMany({ data: lines.map((l) => ({ ...l, budgetId: id })) });
        }
    ...
    async approveBudget(id) {
        ...
        if (!b)
            throw new NotFoundException('Budget tidak ditemukan');
        if (b.status !== 'DRAFT')
            throw new BadRequestException('Hanya budget DRAFT yang bisa di-approve');
    ...
    async getBudgetVsActual(budgetId) {
        ...
        for (const line of budget.lines) {
            if (!grouped.has(line.accountId))
                grouped.set(line.accountId, new Map());
        ...
        for (const [accountId, monthMap] of grouped) {
            ...
            if (!account)
                continue;
            const row = { accountId, accountCode: account.code, accountName: account.name, bulanData: {} };
    ...
    async checkBudgetAvailability(accountId, amount, bulan, tahun) {
