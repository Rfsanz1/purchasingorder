...
let AssetService = class AssetService {
    ...
    async getAssets(query) {
        ...
        const where = {};
        if (search)
            where.OR = [{ nama: { contains: search, mode: 'insensitive' } }, { kode: { contains: search, mode: 'insensitive' } }];
        if (kategori)
    ...
    // ─── DEPRESIASI MANUAL ────────────────────────────────────────────────────
    async calculateDepreciation(assetId, bulan, tahun) {
        ...
        if (asset.status !== 'ACTIVE')
            throw new BadRequestException('Aset tidak aktif');
    ...
    async runMonthlyDepreciation(bulan, tahun) {
    ...
    // ─── DISPOSAL ────────────────────────────────────────────────────────────
    async disposeAsset(assetId, tanggalDisposal, nilaiDisposal, note) {
        const asset = await this.getAsset(assetId);
        if (asset.status !== 'ACTIVE')
            throw new BadRequestException('Aset sudah dilepas');
    ...
    // ─── ASSET REGISTER ──────────────────────────────────────────────────────
    async getAssetRegister(asOfDate) {
