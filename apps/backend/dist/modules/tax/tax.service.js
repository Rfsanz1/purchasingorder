...
let TaxService = class TaxService {
    ...
    async findOne(id) {
        ...
        if (!tax)
            throw new NotFoundException(`Pajak ID ${id} tidak ditemukan`);
        return tax;
    }
    async create(dto) {
    ...
    calculatePPN(amount, taxRate = 11) {
        if (amount < 0)
    ...
    calculatePPh21(grossSalary, statusPajak) {
        const ptkp = PTKP_2024[statusPajak];
        if (ptkp === undefined) {
            throw new BadRequestException(`Status pajak "${statusPajak}" tidak valid. Gunakan: ${Object.keys(PTKP_2024).join(', ')}`);
        }
    ...
    calculatePPh23(amount, jenis) {
        const rate = PPH23_RATES[jenis.toLowerCase()];
        if (rate === undefined) {
            throw new BadRequestException(`Jenis PPh23 "${jenis}" tidak valid. Pilihan: ${Object.keys(PPH23_RATES).join(', ')}`);
        }
        const pph23 = Math.round(amount * rate);
        return { bruto: amount, rate: rate * 100, pph23, jenis };
    }
    calculatePPh4a2(amount, jenis) {
        const rate = PPH4A2_RATES[jenis.toLowerCase()];
        if (rate === undefined) {
            throw new BadRequestException(`Jenis PPh 4(2) "${jenis}" tidak valid. Pilihan: ${Object.keys(PPH4A2_RATES).join(', ')}`);
        }
        const pph4a2 = Math.round(amount * rate);
        return { bruto: amount, rate: rate * 100, pph4a2, jenis };
    }
    async createTaxLine(referenceId, referenceType, taxId, baseAmount, taxAmount, tipe) {
        return this.prisma.taxLine.create({
            data: { referenceId, referenceType, taxId, baseAmount, taxAmount, tipe },
        });
    }
    async getTaxLinesByReference(referenceId) {
