...
let KledoService = KledoService_1 = class KledoService {
    http;
    prisma;
    logger = new Logger(KledoService_1.name);
    baseUrl;
    token;
    ...
    async getStatus() {
        if (!this.token)
            return { connected: false, message: 'KLEDO_TOKEN tidak dikonfigurasi' };
        try {
    ...
    async findOrCreateContact(name, phone) {
        try {
            const res = await firstValueFrom(this.http.get(`${this.baseUrl}/finance/contacts`, {
        ...
        try {
            const createRes = await firstValueFrom(this.http.post(`${this.baseUrl}/finance/contacts`, { name, phone: phone ?? null, type_id: 4, is_customer: 1 }, { headers: this.headers }));
    ...
    async createInvoice(dto) {
        ...
        try {
            ...
            if (items.length === 0)
                return { success: false, message: 'Tidak ada produk dengan Kledo Product ID' };
            const payload = {
    ...
    isSpmBrand(brand) { return brand?.toUpperCase() in SPM_BRAND_PIC; }
    withMargin(price, margin = 0.15) { return Math.ceil(price * (1 + margin)); }
    /** ─── BACKGROUND SYNC HELPERS ─── */
    /** Jalankan fn di background, update log saat selesai */
    runBackground(logId, fn) {
        setImmediate(async () => {
            try {
    ...
    async fetchPage(path, page, perPage = 100, retries = 3) {
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                const res = await firstValueFrom(this.http.get(`${this.baseUrl}${path}`, {
                    headers: this.headers,
    ...
    async syncProducts() {
        if (!this.token)
            return { jobId: '', message: 'KLEDO_TOKEN tidak dikonfigurasi' };
        const PER_PAGE = 100;
    ...
    async syncAll() {
        if (!this.token)
            throw new Error('KLEDO_TOKEN tidak dikonfigurasi');
...
KledoService = KledoService_1 = __decorate([
    Injectable(),
