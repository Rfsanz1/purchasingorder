...
let PayrollService = class PayrollService {
    ...
    async calculatePayroll(periodId) {
        ...
        if (period.status === 'PAID')
            throw new BadRequestException('Periode sudah dibayar');
        ...
        for (const emp of employees) {
            ...
            for (const comp of components) {
                ...
                if (amount !== 0) {
                    ...
                    if (comp.tipe === 'ALLOWANCE' || comp.tipe === 'OVERTIME')
                        totalTunjangan += Math.max(0, amount);
                    if (comp.tipe === 'DEDUCTION')
                        totalPotongan += Math.abs(amount);
                }
            }
            // BPJS
    ...
    async approvePayroll(periodId) {
        ...
        if (negative.length > 0)
            throw new BadRequestException(`${negative.length} slip memiliki gaji negatif`);
    ...
    async processPayment(periodId) {
        ...
        if (period.status !== 'APPROVED')
            throw new BadRequestException('Periode harus di-approve dulu');
    ...
    async createComponent(dto) { return this.prisma.payrollComponent.create({ data: dto }); }
    async updateComponent(id, dto) { return this.prisma.payrollComponent.update({ where: { id }, data: dto }); }
    // ─── BPJS REPORT ─────────────────────────────────────────────────────────
    async getBPJSReport(periodId) {
    ...
    // ─── BPJS CONFIG ─────────────────────────────────────────────────────────
    async getBPJSConfig(employeeId) {
        return this.prisma.bPJSConfig.findUnique({ where: { employeeId } });
    }
    async upsertBPJSConfig(employeeId, dto) {
    ...
    async bankExport(periodId, format = 'csv') {
        const slips = await this.prisma.payrollSlip.findMany({
    ...
    async sendAllSlipEmails(periodId) {
        const slips = await this.prisma.payrollSlip.findMany({
            where: { periodId },
    ...
    async findOrCreateAccountId(code, name, type) {
        const acc = await this.prisma.account.findFirst({ where: { code } });
        if (acc)
            return acc.id;
        const created = await this.prisma.account.create({ data: { code, name, type: type } });
        return created.id;
    }
};
