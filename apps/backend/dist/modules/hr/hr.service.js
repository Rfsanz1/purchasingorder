...
let HrService = class HrService {
    ...
    async createEmployee(dto) { return this.prisma.employee.create({ data: dto }); }
    async updateEmployee(id, dto) { return this.prisma.employee.update({ where: { id }, data: dto }); }
    async deleteEmployee(id) { return this.prisma.employee.update({ where: { id }, data: { status: 'nonaktif' } }); }
    async getPayrolls(query) {
        ...
        if (status)
            where.status = status;
        if (periode)
            where.periode = { contains: periode };
        const [data, total] = await Promise.all([
    ...
    async updatePayroll(id, dto) { return this.prisma.payroll.update({ where: { id }, data: dto }); }
    async getAttendances(query) {
        ...
        if (employeeId)
            where.employeeId = employeeId;
        if (tanggal)
            where.tanggal = { gte: new Date(tanggal) };
        const [data, total] = await Promise.all([
    ...
    async createAttendance(dto) { return this.prisma.attendance.create({ data: dto }); }
    async getStats() {
    ...
    async createContract(dto) {
        if (!dto.employeeId)
            throw new BadRequestException('employeeId harus diisi');
        if (!dto.startDate)
            throw new BadRequestException('startDate harus diisi');
    ...
    async createShift(dto) {
        if (!dto.nama)
            throw new BadRequestException('Nama shift harus diisi');
    ...
    async createMutasi(dto) {
        if (!dto.employeeId)
            throw new BadRequestException('employeeId harus diisi');
        if (!dto.effectiveDate)
            throw new BadRequestException('effectiveDate harus diisi');
    ...
    async importCsv(rows) {
        if (!Array.isArray(rows) || rows.length === 0)
            throw new BadRequestException('Data CSV tidak valid atau kosong');
        const results = [];
    ...
    async importAttendanceCsv(rows) {
        if (!Array.isArray(rows) || rows.length === 0)
            throw new BadRequestException('Data CSV tidak valid atau kosong');
    ...
    // ─── ATTENDANCE REPORTS ───────────────────────────────────────────────────
    async getAttendanceReport(employeeId, month, year) {
    ...
    async getAttendanceSummary(month, year, departemenId) {
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        const where = { tanggal: { gte: startDate, lte: endDate }, employee: { status: 'aktif' } };
        if (departemenId)
            where.employee.departemen = departemenId;
        const attendances = await this.prisma.attendance.findMany({
            where,
    ...
    // ─── SHIFT ASSIGNMENT ────────────────────────────────────────────────────
    async assignShift(employeeId, shiftId) {
    ...
    async createLeaveType(dto) {
        if (!dto.nama)
            throw new BadRequestException('Nama jenis cuti harus diisi');
    ...
    async createLeaveRequest(dto) {
        if (!dto.employeeId)
            throw new BadRequestException('employeeId harus diisi');
        if (!dto.leaveTypeId)
            throw new BadRequestException('leaveTypeId harus diisi');
        if (!dto.dateFrom)
            throw new BadRequestException('dateFrom harus diisi');
        if (!dto.dateTo)
            throw new BadRequestException('dateTo harus diisi');
    ...
    // ─── LEAVE ALLOCATIONS ───────────────────────────────────────────────────
    async getLeaveAllocations(employeeId, year) {
        const where = {};
        if (employeeId)
            where.employeeId = employeeId;
        if (year)
    ...
    async createLeaveAllocation(dto) {
        if (!dto.employeeId)
            throw new BadRequestException('employeeId harus diisi');
        if (!dto.leaveTypeId)
            throw new BadRequestException('leaveTypeId harus diisi');
        if (!dto.numberOfDays)
            throw new BadRequestException('numberOfDays harus diisi');
        if (!dto.year)
            throw new BadRequestException('year harus diisi');
    ...
    // ─── LEAVE CALENDAR ──────────────────────────────────────────────────────
    async getLeaveCalendar(month, year, departemenId) {
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
    ...
    async createEmployeeContract(employeeId, dto) {
        if (!dto.startDate)
            throw new BadRequestException('startDate harus diisi');
    ...
    async updateEmployeeContract(contractId, dto) {
    ...
    // ─── EMPLOYEE TRANSFER ────────────────────────────────────────────────────
    async transferEmployee(employeeId, dto) {
