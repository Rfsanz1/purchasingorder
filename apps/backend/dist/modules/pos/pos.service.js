...
let PosService = class PosService {
    ...
    async resumeTransaction(id, dto) {
    ...
    async returnTransaction(id, dto) {
    ...
    async getActiveSession(currentUser) {
    ...
    async openSession(dto, currentUser) {
        ...
        if (!user)
            throw new NotFoundException('Tidak ada POS user aktif');
    ...
    async getSession(id) {
        ...
        const breakdown = { cash: 0, transfer: 0, card: 0, qris: 0 };
    ...
    async redeemLoyalty(dto) {
        const { customerId, points } = dto;
        if (!customerId || !points)
            throw new BadRequestException('customerId and points are required');
        const record = await this.getCustomerLoyalty(customerId);
        if (record.points < Number(points))
            throw new BadRequestException('Saldo poin tidak mencukupi');
    ...
    async getReportsProducts(query) {
    ...
    async getReportsPayments(query) {
    ...
    async getReportsCashiers(query) {
        const grouped = await this.prisma.posSale.groupBy({
            by: ['posUserId'],
            _sum: { grandTotal: true },
            _count: { id: true },
    ...
    prepareSaleData(dto, allowNoItems = true) {
    ...
    extractCashAmount(sale) {
        const payments = Array.isArray(sale.splitPayments) ? sale.splitPayments : [];
        if (payments.length) {
            return payments.filter((payment) => ['tunai', 'cash'].includes(String(payment.method ?? '').toLowerCase())).reduce((sum, payment) => sum + Number(payment.amount ?? 0), 0);
        }
    ...
    prepareSaleAmounts(dto) {
        ...
        const cashAmount = splitPayments
            ? splitPayments.filter((payment) => ['tunai', 'cash'].includes(String(payment.method ?? '').toLowerCase())).reduce((sum, payment) => sum + Number(payment.amount ?? 0), 0)
    ...
    async adjustCustomerLoyalty(customerId, earned, used) {
        if (!customerId)
            return;
    ...
    async createSessionCloseJournal(session, cashRevenue) {
