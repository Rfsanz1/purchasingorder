...
let HelpdeskService = class HelpdeskService {
    ...
    async createTeam(dto) { return this.prisma.helpdeskTeam.create({ data: dto }); }
    ...
    async createTicket(dto) {
        ...
        if (teamId) {
            ...
            if (team) {
                slaDeadline = new Date(Date.now() + team.slaHours * 3600000);
            }
        }
        return this.prisma.helpdeskTicket.create({ data: { ...dto, noTicket, slaDeadline } });
    }
    async updateTicket(id, dto) { return this.prisma.helpdeskTicket.update({ where: { id }, data: dto }); }
    async closeTicket(id, rating, ratingComment) {
