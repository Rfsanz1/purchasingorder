...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', opacity: mounted ? 1 : 0, transition:'opacity .4s' }}>
      <div className="hidden lg:flex lg:w-[48%]" style={{ background:'linear-gradient(145deg,#C2410C 0%,#E64A19 50%,#FB923C 100%)', flexDirection:'column', justifyContent:'space-between', padding:'40px 36px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ width:36, height:36, borderRadius:10, background:'rgba(255,255,255,.95)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, color:'#E64A19', fontSize:15 }}>G</div>
        ...
        <div style={{ position:'relative' }}>
          <h1 style={{ color:'#fff', fontSize:'1.9rem', fontWeight:800, lineHeight:1.2, margin:'0 0 12px' }}>Kasir cepat,<br /><span style={{ color:'#FED7AA' }}>transaksi mudah.</span></h1>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10 }}>
          {[
            { icon: Monitor, label:'Buka Kasir', color:'#FDBA74' },
            { icon: ShoppingCart, label:'Transaksi', color:'#FED7AA' },
            { icon: CreditCard, label:'Pembayaran', color:'#FFEDD5' },
            { icon: BarChart2, label:'Laporan Sesi', color:'#FFF7ED' },
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', backgroundColor:'#fff', padding:'40px 44px' }}>
        <div style={{ width:'100%', maxWidth:360 }}>
          <div style={{ marginBottom:32 }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:6, borderRadius:100, padding:'4px 12px', backgroundColor:'#FFF7ED', border:'1px solid #FED7AA', color:'#C2410C', fontSize:11, fontWeight:600, marginBottom:16 }}>
              <Monitor size={10} /> POS App — Kasir
            </div>
            <h2 style={{ fontSize:'2rem', fontWeight:800, color:'#7C2D12', margin:'0 0 8px' }}>Mulai Sesi Kasir</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div>
              ...
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)}
                ...
                style={{ width:'100%', outline:'none', fontSize:13.5, borderRadius:12, padding:'11px 16px', border: focused==='email' ? '1.5px solid #FB923C' : '1.5px solid #E5E7EB', boxShadow: focused==='email' ? '0 0 0 4px rgba(251,146,60,.15)' : '0 1px 3px rgba(0,0,0,.04)', backgroundColor:'#FAFAFA', color:'#111827', transition:'all .2s', boxSizing:'border-box' }} />
            </div>
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input type={showPass ? 'text' : 'password'} required value={password} onChange={e => setPassword(e.target.value)}
                  ...
                  style={{ width:'100%', outline:'none', fontSize:13.5, borderRadius:12, padding:'11px 44px 11px 16px', border: focused==='pass' ? '1.5px solid #FB923C' : '1.5px solid #E5E7EB', boxShadow: focused==='pass' ? '0 0 0 4px rgba(251,146,60,.15)' : '0 1px 3px rgba(0,0,0,.04)', backgroundColor:'#FAFAFA', color:'#111827', transition:'all .2s', boxSizing:'border-box' }} />
            ...
            <button type="submit" disabled={loading} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'13px 20px', borderRadius:14, background:'linear-gradient(135deg,#C2410C,#FB923C)', color:'#fff', fontSize:14, fontWeight:700, border:'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? .75 : 1, boxShadow:'0 6px 24px rgba(194,65,12,.35)', marginTop:4 }}>
