...
const NAV = [
  ...
  { href: '/smart-order',    label: 'Smart Order',  icon: Zap },
  { href: '/quotations',     label: 'Quotation',    icon: FileText },
  { href: '/orders',         label: 'Sales Order',  icon: ShoppingCart },
  { href: '/faktur',         label: 'Faktur',        icon: CreditCard },
  { href: '/customers',      label: 'Pelanggan',    icon: Users },
  { href: '/crm/pipeline',   label: 'Pipeline CRM', icon: LayoutGrid },
  { href: '/crm/leads',      label: 'Leads',        icon: Star },
  { href: '/crm/activities', label: 'Aktivitas',    icon: Phone },
...
export function SalesLayout({ children, title, subtitle }: SalesLayoutProps) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', backgroundColor: C.bg, opacity: mounted ? 1 : 0, transition: 'opacity .3s' }}>
      ...
      {sidebarOpen && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 40, backgroundColor: 'rgba(0,0,0,.4)' }}
      ...
      {sidebarOpen && (
        <aside style={{
          ...
          <SidebarContents
            ...
            user={user} onNav={h => { router.push(h); setSidebarOpen(false); }}
      ...
      >
        ...
        <SidebarContents
          nav={NAV} pathname={pathname ?? ''}
          user={user} onNav={h => router.push(h)} onLogout={handleLogout}
        />
      </aside>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px', borderBottom: '1px solid rgba(255,255,255,.12)' }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: '#fff', margin: 0 }}>Sales App</p>
