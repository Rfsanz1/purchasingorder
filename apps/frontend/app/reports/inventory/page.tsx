  ...
  return (
    <AppShell {...REPORTS_CONFIG} navItems={REPORTS_NAV} activeHref="/reports/inventory">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Laporan Inventaris</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Stok masuk/keluar, nilai inventory, perputaran</p></div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}><h2 className="text-sm font-bold" style={{ color: '#433C50' }}>Produk dengan Pergerakan Terbanyak (Mei)</h2></div>
              ...
              <tbody>
                {TOP_MOVES.map((m, i) => (
                  <tr key={m.name} style={{ borderBottom: i < TOP_MOVES.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-sm font-medium" style={{ color: '#433C50' }}>{m.name}</td>
                    ...
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{m.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
