  ...
  return (
    <AppShell {...REPORTS_CONFIG} navItems={REPORTS_NAV} activeHref="/reports/sales">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Laporan Penjualan</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Revenue, order, dan performa sales per periode</p></div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}><h2 className="text-sm font-bold" style={{ color: '#433C50' }}>Rekap per Bulan</h2></div>
              ...
              <tbody>
                {DATA.map((d, i) => (
                  <tr key={d.bulan} style={{ borderBottom: i < DATA.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{d.order}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: '#4CAF50' }}>{d.revenue}</td>
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{d.pelanggan}</td>
