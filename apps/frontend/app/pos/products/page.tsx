    ...
    <AppShell {...POS_CONFIG} navItems={POS_NAV} activeHref="/pos/products">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Produk POS</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Produk yang tersedia di kasir</p></div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari produk..." />
            </div>
          </div>
                  ...
                  <tr key={p.sku} style={{ borderBottom: i < PRODUCTS.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium" style={{ color: '#433C50' }}>{p.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-3.5 text-xs font-mono" style={{ color: '#A5A3AE' }}>{p.sku}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: '#433C50' }}>{p.price}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: p.stock < 10 ? '#EA5455' : '#433C50' }}>{p.stock}</td>
                    <td className="px-6 py-3.5">
                      <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold" style={{ color: p.active ? '#4CAF50' : '#A5A3AE', backgroundColor: p.active ? 'rgba(76,175,80,.1)' : 'rgba(165,163,174,.12)' }}>
