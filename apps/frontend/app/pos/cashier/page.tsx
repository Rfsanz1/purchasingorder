    ...
    <AppShell {...POS_CONFIG} navItems={POS_NAV} activeHref="/pos/cashier">
      <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <h1 className="text-lg font-bold mb-4" style={{ color: '#433C50' }}>Kasir — Pilih Produk</h1>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {PRODUCTS.map(p => (
              <button key={p.id} onClick={() => addToCart(p)} className="bg-white rounded-2xl p-4 text-left transition"
                ...
                <p className="text-xs font-semibold leading-tight" style={{ color: '#433C50' }}>{p.name}</p>
        ...
        <div className="w-80 flex flex-col bg-white" style={{ borderLeft: '1.5px solid #EDE8F5' }}>
          <div className="px-4 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold" style={{ color: '#433C50' }}>Keranjang ({cart.length} item)</h2>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {cart.length === 0 ? (
              <p className="text-xs text-center py-8" style={{ color: '#A5A3AE' }}>Belum ada item dipilih</p>
            ) : cart.map(c => (
              <div key={c.id} className="flex items-center justify-between py-2" style={{ borderBottom: '1px solid #F5F2FB' }}>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: '#433C50' }}>{c.name}</p>
                  <p className="text-xs" style={{ color: '#A5A3AE' }}>Rp {c.price.toLocaleString('id-ID')}</p>
                </div>
                <div className="flex items-center gap-1.5 ml-2">
                  <button onClick={() => updateQty(c.id, -1)} className="flex h-6 w-6 items-center justify-center rounded-full" style={{ backgroundColor: '#F5F2FB' }}><Minus className="h-3 w-3" style={{ color: '#433C50' }} /></button>
                  <span className="text-xs font-bold w-4 text-center" style={{ color: '#433C50' }}>{c.qty}</span>
          ...
          <div className="p-4 space-y-3" style={{ borderTop: '1.5px solid #EDE8F5' }}>
            <div className="flex justify-between">
              <span className="text-sm font-medium" style={{ color: '#A5A3AE' }}>Subtotal</span>
              <span className="text-sm font-bold" style={{ color: '#433C50' }}>Rp {total.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm font-medium" style={{ color: '#A5A3AE' }}>PPN 11%</span>
              <span className="text-sm font-bold" style={{ color: '#433C50' }}>Rp {Math.round(total * 0.11).toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between pt-2" style={{ borderTop: '1px solid #EDE8F5' }}>
              <span className="text-sm font-bold" style={{ color: '#433C50' }}>TOTAL</span>
