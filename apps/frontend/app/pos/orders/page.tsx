  ...
  return (
    <AppShell {...POS_CONFIG} navItems={POS_NAV} activeHref="/pos/orders">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Order POS Hari Ini</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>87 transaksi · Rp 14.2 Jt total</p></div>
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari order..." />
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {ORDERS.map((o, i) => {
                  const ms = METHOD_STYLE[o.method] ?? { color: '#A5A3AE', bg: 'rgba(165,163,174,.12)' };
                  return (
                    <tr key={o.no} style={{ borderBottom: i < ORDERS.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                      ...
                      <td className="px-6 py-3.5 text-xs" style={{ color: '#A5A3AE' }}>{o.kasir}</td>
                      ...
                      <td className="px-6 py-3.5 text-xs" style={{ color: '#A5A3AE' }}>{o.time}</td>
                    </tr>
                  );
                })}
              </tbody>
