    ...
    <AppShell {...CRM_CONFIG} navItems={CRM_NAV} activeHref="/crm/opportunities">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Opportunity</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola peluang penjualan dan pipeline CRM</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari opportunity..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <select className="rounded-lg px-3 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={stage} onChange={e => setStage(e.target.value)}>
              <option value="">Semua Stage</option>
              {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <button onClick={load} className="p-2 rounded-lg" style={{ border: '1px solid #EDE8F5', color: '#A5A3AE' }}><RefreshCw className="h-4 w-4" /></button>
          </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-12 text-sm" style={{ color: '#A5A3AE' }}>Belum ada opportunity</td></tr>
                  ) : items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-3 font-semibold" style={{ color: '#433C50' }}>{item.name ?? item.title ?? '-'}</td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{item.customerName ?? item.customer ?? '-'}</td>
                        ...
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs" style={{ color: '#6D6777' }}>{item.probability ?? 20}%</span>
                          </div>
                        </td>
                        <td className="px-6 py-3 font-semibold" style={{ color: '#433C50' }}>{Number(item.expectedRevenue ?? 0).toLocaleString('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 })}</td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{item.deadline ? new Date(item.deadline).toLocaleDateString('id-ID') : '-'}</td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{item.salesperson ?? '-'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-hidden" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Tambah Opportunity</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Stage</label>
                    <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.stage} onChange={e => setForm(f => ({ ...f, stage: e.target.value }))}>
                      {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Probabilitas (%)</label>
                    <input type="number" min={0} max={100} className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.probability} onChange={e => setForm(f => ({ ...f, probability: +e.target.value }))} onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = '#EDE8F5'} />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Catatan</label>
                  <textarea className="w-full rounded-lg px-4 py-2.5 text-sm resize-none" rows={3} style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Catatan opportunity..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
                </div>
