  ...
  return (
    <AppShell {...PURCHASING_CONFIG} navItems={PURCHASING_NAV} activeHref="/purchasing/reports">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Laporan Pembelian</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Analitik PO dan performa supplier</p></div>
              ...
              <tbody>
                {MONTHLY.map((m, i) => (
                  <tr key={m.bulan} style={{ borderBottom: i < MONTHLY.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{m.po}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: '#EA5455' }}>{m.nilai}</td>
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{m.supplier}</td>
