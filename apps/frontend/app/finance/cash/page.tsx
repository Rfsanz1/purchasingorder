    ...
    <AppShell {...ACCOUNTING_CONFIG} navItems={ACCOUNTING_NAV} activeHref="/finance/cash">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Kas Keluar / Masuk</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Transaksi kas harian</p></div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
            <p className="text-xs font-medium" style={{ color: '#A5A3AE' }}>Saldo Kas Saat Ini</p>
            <p className="text-2xl font-bold mt-1" style={{ color: '#433C50' }}>Rp 84.760.000</p>
          </div>
          <div className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
            <p className="text-xs font-medium" style={{ color: '#A5A3AE' }}>Net Kas Bulan Ini</p>
              ...
              <tbody>
                {TRANSACTIONS.map((t, i) => (
                  <tr key={t.ref} style={{ borderBottom: i < TRANSACTIONS.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-xs font-mono" style={{ color: '#A5A3AE' }}>{t.ref}</td>
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{t.desc}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: t.type === 'in' ? '#4CAF50' : '#EA5455' }}>{t.type === 'in' ? '+' : '-'}{t.amount}</td>
                    <td className="px-6 py-3.5 text-xs" style={{ color: '#A5A3AE' }}>{t.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
