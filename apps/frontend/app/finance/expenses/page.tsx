    ...
    <AppShell {...ACCOUNTING_CONFIG} navItems={ACCOUNTING_NAV} activeHref="/finance/expenses">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Pengeluaran</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola biaya operasional dan pengeluaran</p></div>
              ...
              <tbody>
                {EXPENSES.map((e, i) => (
                  <tr key={e.ref} style={{ borderBottom: i < EXPENSES.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-sm" style={{ color: '#433C50' }}>{e.desc}</td>
                    ...
                    <td className="px-6 py-3.5 text-xs" style={{ color: '#A5A3AE' }}>{e.date}</td>
