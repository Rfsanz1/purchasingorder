    ...
    <AppShell {...ACCOUNTING_CONFIG} navItems={ACCOUNTING_NAV} activeHref="/finance/aged-receivable">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Aging Piutang (Aged Receivable)</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Analisa umur piutang pelanggan berdasarkan tanggal jatuh tempo</p>
          </div>
          <div className="flex gap-2">
            <input type="date" value={asOfDate} onChange={e => setAsOfDate(e.target.value)} className="rounded-lg px-3 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} />
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari pelanggan..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE8F5', backgroundColor: '#F8F7FC' }}>
                  <th className="px-6 py-3 text-left text-xs font-semibold" style={{ color: '#A5A3AE' }}>Pelanggan</th>
                  {COLUMNS.map(col => (
                    <th key={col.key} className="px-4 py-3 text-right text-xs font-semibold" style={{ color: col.color }}>{col.label}</th>
                  ))}
                  <th className="px-6 py-3 text-right text-xs font-semibold" style={{ color: '#433C50' }}>Total</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(d => (
                  <tr key={d.customer} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                    <td className="px-6 py-3 font-medium" style={{ color: '#433C50' }}>
                    ...
                    {COLUMNS.map(col => (
                      <td key={col.key} className="px-4 py-3 text-right text-xs" style={{ color: (d as any)[col.key] > 0 ? col.color : '#A5A3AE' }}>
                        {(d as any)[col.key] > 0 ? fmt((d as any)[col.key]) : '-'}
                      </td>
                    ))}
                    <td className="px-6 py-3 text-right font-bold text-sm" style={{ color: '#433C50' }}>{fmt(d.total)}</td>
                  </tr>
                ))}
                <tr style={{ borderTop: '2px solid #EDE8F5', backgroundColor: '#F8F7FC' }}>
                  <td className="px-6 py-3 font-bold text-sm" style={{ color: '#433C50' }}>TOTAL</td>
                  {COLUMNS.map(col => (
                    <td key={col.key} className="px-4 py-3 text-right font-bold text-sm" style={{ color: col.color }}>{fmt((totals as any)[col.key])}</td>
                  ))}
                  <td className="px-6 py-3 text-right font-bold text-sm" style={{ color: '#433C50' }}>{fmt(totals.total)}</td>
                </tr>
              </tbody>
            </table>
          </div>
