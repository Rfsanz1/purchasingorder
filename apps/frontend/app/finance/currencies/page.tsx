    ...
    <AppShell {...ACCOUNTING_CONFIG} navItems={ACCOUNTING_NAV} activeHref="/finance/currencies">
      <div className="p-6 space-y-6 max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Multi Mata Uang</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola kurs valuta asing untuk transaksi multi-currency</p>
          </div>
            ...
            return (
              <div key={c.code} className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-lg" style={{ color: '#433C50' }}>{c.code}</span>
                ...
                <p className="text-xs mt-1" style={{ color: '#A5A3AE' }}>{c.name}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <h3 className="font-semibold text-sm" style={{ color: '#433C50' }}>Tabel Kurs — {new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</h3>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {CURRENCIES.map(c => {
                  ...
                  return (
                    <tr key={c.code} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium" style={{ color: '#433C50' }}>{c.name}</td>
                      <td className="px-4 py-3 text-sm font-bold" style={{ color: '#6D6777' }}>{c.symbol}</td>
                      <td className="px-4 py-3">
                        {!isBase ? (
                          editRates[c.code] !== undefined ? (
                            <input type="number" className="w-24 rounded px-2 py-1 text-sm font-bold" style={{ border: '1.5px solid' + C, outline: 'none', color: '#433C50' }} value={editRates[c.code]} onChange={e => setEditRates(r => ({ ...r, [c.code]: e.target.value })} />
                          ) : (
                            <span className="font-bold" style={{ color: '#433C50' }}>Rp {fmt(c.rate)}</span>
                          )
                        ) : (
                          <span className="font-bold" style={{ color: '#4CAF50' }}>1.00 (Base)</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#A5A3AE' }}>{isBase ? '-' : `Rp ${fmt(c.rate_prev)}`}</td>
                      <td className="px-4 py-3">
                        {!isBase && (
                      ...
                      <td className="px-4 py-3 text-xs" style={{ color: '#A5A3AE' }}>{c.last_update}</td>
                      <td className="px-4 py-3">
                        {!isBase && (
                          editRates[c.code] !== undefined ? (
                            <div className="flex gap-1.5">
                              ...
                              <button onClick={() => setEditRates(r => { const n = { ...r }; delete n[c.code]; return n; })} className="text-xs font-semibold px-2 py-1.5 rounded-lg" style={{ backgroundColor: 'rgba(158,158,158,.1)', color: '#6D6777' }}>Batal</button>
                            </div>
                          ) : (
