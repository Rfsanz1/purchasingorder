    ...
    <AppShell {...SERVICE_CONFIG} navItems={SERVICE_NAV} activeHref="/service/work-orders">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Work Order Servis</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola pekerjaan servis dan reparasi perangkat</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari pelanggan atau perangkat..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-xs max-w-[150px] truncate" style={{ color: '#6D6777' }}>{item.problem}</td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#433C50' }}>{item.technician || '-'}</td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{new Date(item.received).toLocaleDateString('id-ID')}</td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.est_done ? new Date(item.est_done).toLocaleDateString('id-ID') : '-'}</td>
                      <td className="px-4 py-3 font-semibold text-xs" style={{ color: '#433C50' }}>{fmt(item.est_cost)}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-y-auto" style={{ maxHeight: '90vh', boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buat Work Order Baru</h2>
