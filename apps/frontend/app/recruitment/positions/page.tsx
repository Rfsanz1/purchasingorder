    ...
    <AppShell {...RECRUITMENT_CONFIG} navItems={RECRUITMENT_NAV} activeHref="/recruitment/positions">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Lowongan Pekerjaan</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola posisi yang sedang dibuka dan alur rekrutmen</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari posisi atau departemen..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
                ...
                return (
                  <div key={pos.id} className="rounded-xl p-5 transition-all hover:shadow-md cursor-pointer" style={{ border: '1.5px solid #EDE8F5' }}>
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-bold" style={{ color: '#433C50' }}>{pos.title}</p>
                        <p className="text-xs mt-0.5" style={{ color: '#A5A3AE' }}>{pos.department}</p>
                      </div>
                    ...
                    <p className="text-xs mt-2" style={{ color: '#A5A3AE' }}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-y-auto" style={{ maxHeight: '90vh', boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buka Lowongan Baru</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Tipe Kerja</label>
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Persyaratan</label>
                  <textarea className="w-full rounded-lg px-4 py-2.5 text-sm resize-none" rows={3} style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Pendidikan min, pengalaman, skill yang dibutuhkan..." value={form.requirements} onChange={e => setForm(f => ({ ...f, requirements: e.target.value }))} />
                </div>
