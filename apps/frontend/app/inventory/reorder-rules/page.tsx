    ...
    <AppShell {...INVENTORY_CONFIG} navItems={INVENTORY_NAV} activeHref="/inventory/reorder-rules">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Reorder Rules</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Atur aturan pemesanan ulang otomatis saat stok mencapai minimum</p>
          </div>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.min_qty}</td>
                    <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.max_qty}</td>
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.supplier}</td>
                    <td className="px-4 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Tambah Reorder Rule</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Route</label>
                  <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.route} onChange={e => setForm(f => ({ ...f, route: e.target.value }))}>
