    ...
    <AppShell {...INVENTORY_CONFIG} navItems={INVENTORY_NAV} activeHref="/inventory/lots">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Lot Number & Serial Number</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Lacak lot produksi, serial number, dan tanggal kadaluarsa</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari lot, serial, atau produk..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <select className="rounded-lg px-3 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={type} onChange={e => setType(e.target.value)}>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium" style={{ color: '#433C50' }}>{item.product}</td>
                      <td className="px-4 py-3 font-bold" style={{ color: '#433C50' }}>{item.qty} {item.uom}</td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{new Date(item.received).toLocaleDateString('id-ID')}</td>
                      <td className="px-4 py-3">
                        {item.expiry ? (
                          <div className="flex items-center gap-1">
                            {item.status === 'expiring' && <AlertTriangle className="h-3 w-3 flex-shrink-0" style={{ color: '#FF9800' }} />}
                            <span className="text-xs" style={{ color: item.status === 'expiring' ? '#FF9800' : '#6D6777' }}>
                              {new Date(item.expiry).toLocaleDateString('id-ID')}
                            </span>
                          </div>
                        ) : <span className="text-xs" style={{ color: '#A5A3AE' }}>-</span>}
                      </td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.location}</td>
                      <td className="px-4 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Tambah Lot / Serial Number</h2>
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <div className="flex gap-3">
                    {['lot', 'serial'].map(t => (
                      <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))} className="flex-1 py-2.5 rounded-lg text-sm font-semibold transition" style={{ backgroundColor: form.type === t ? C : 'transparent', color: form.type === t ? '#FFFFFF' : '#6D6777', border: `1.5px solid ${form.type === t ? C : '#EDE8F5'}` }}>
                ...
                {form.type === 'lot' && (
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Tanggal Kadaluarsa</label>
                    <input type="date" className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.expiry} onChange={e => setForm(f => ({ ...f, expiry: e.target.value }))} />
                  </div>
                )}
