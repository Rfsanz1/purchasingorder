    ...
    <AppShell {...INVENTORY_CONFIG} navItems={INVENTORY_NAV} activeHref="/inventory/products/categories">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Kategori Produk</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola kategori dan sub-kategori produk</p></div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {CATEGORIES.map((c) => (
            <div key={c.slug} className="bg-white rounded-2xl p-5 cursor-pointer transition" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}
              ...
              <p className="text-sm font-bold" style={{ color: '#433C50' }}>{c.name}</p>
              <p className="text-xs mt-1" style={{ color: '#A5A3AE' }}>{c.produk} produk</p>
            </div>
          ))}
        </div>
      </div>
