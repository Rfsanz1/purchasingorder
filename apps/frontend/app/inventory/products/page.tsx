    ...
    <AppShell {...INVENTORY_CONFIG} navItems={INVENTORY_NAV} activeHref="/inventory/products">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Produk & Stok</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Katalog produk dan manajemen stok</p></div>
          <div className="flex gap-2">
            <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium" style={{ border: '1.5px solid #EDE8F5', color: '#A5A3AE' }}><RefreshCw className="h-4 w-4" /></button>
        ...
        <div className="flex gap-1 p-1 rounded-xl w-fit" style={{ backgroundColor: '#F5F2FB' }}>
          <a href="/inventory/products" className="px-4 py-1.5 rounded-lg text-xs font-semibold" style={{ backgroundColor: 'white', color: '#433C50', boxShadow: '0 1px 3px rgba(47,43,61,.1)' }}>Semua Produk</a>
          <a href="/inventory/products/categories" className="px-4 py-1.5 rounded-lg text-xs font-semibold" style={{ color: '#A5A3AE' }}>Kategori</a>
        </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari produk..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm" style={{ color: '#A5A3AE' }}>Memuat produk...</td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm" style={{ color: '#A5A3AE' }}>Tidak ada produk ditemukan</td></tr>
                  ...
                  <tr key={p.id ?? i} style={{ borderBottom: i < data.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-xs font-mono" style={{ color: '#A5A3AE' }}>{p.code || p.sku || '–'}</td>
