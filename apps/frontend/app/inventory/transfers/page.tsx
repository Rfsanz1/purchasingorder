    ...
    <AppShell {...INVENTORY_CONFIG} navItems={INVENTORY_NAV} activeHref="/inventory/transfers">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Transfer Stok</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Pindahkan stok antar gudang atau area produksi</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari lokasi asal atau tujuan..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium" style={{ color: '#433C50' }}>{item.from}</td>
                      <td className="px-4 py-3" style={{ color: '#433C50' }}>
                      ...
                      <td className="px-4 py-3 text-center text-xs" style={{ color: '#433C50' }}>{item.items}</td>
                      <td className="px-4 py-3 font-bold text-xs" style={{ color: '#433C50' }}>{fmt(item.total_qty)} {item.unit}</td>
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{item.pic}</td>
                      <td className="px-4 py-3 text-xs max-w-[150px] truncate" style={{ color: '#A5A3AE' }}>{item.notes}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-y-auto" style={{ maxHeight: '90vh', boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buat Transfer Stok</h2>
                <button onClick={() => setShowForm(false)} style={{ color: '#A5A3AE' }}><X className="h-5 w-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Dari Gudang *</label>
                    <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.from} onChange={e => setForm(f => ({ ...f, from: e.target.value }))}>
                      <option value="">Pilih gudang asal...</option>
                      {LOCATIONS.map(l => <option key={l} value={l}>{l}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Ke Gudang *</label>
                    <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.to} onChange={e => setForm(f => ({ ...f, to: e.target.value }))}>
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Tanggal Transfer</label>
                  <input type="date" className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Catatan</label>
                  <textarea className="w-full rounded-lg px-4 py-2.5 text-sm resize-none" rows={2} style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Keterangan transfer..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-semibold" style={{ color: '#433C50' }}>Item Transfer</label>
