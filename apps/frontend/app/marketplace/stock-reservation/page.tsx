    ...
    <AppShell {...MARKETPLACE_CONFIG} navItems={MARKETPLACE_NAV} activeHref="/marketplace/stock-reservation">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Reservasi Stok Marketplace</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Pantau dan kelola stok yang direservasi untuk pesanan marketplace</p>
          </div>
                ...
                {SAMPLE.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-xs font-mono" style={{ color: '#6D6777' }}>{item.order_id}</td>
                      <td className="px-4 py-3 font-bold text-center" style={{ color: '#433C50' }}>{item.reserved_qty}</td>
                      ...
                      <td className="px-4 py-3 text-xs" style={{ color: '#6D6777' }}>{new Date(item.reserved_until).toLocaleDateString('id-ID')}</td>
                      <td className="px-4 py-3">
