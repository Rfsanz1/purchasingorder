    ...
    <AppShell {...MARKETPLACE_CONFIG} navItems={MARKETPLACE_NAV} activeHref="/marketplace/price-sync">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Sinkronisasi Harga Marketplace</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Sinkronkan harga dan stok ke semua platform marketplace</p>
          </div>
          <div className="flex items-center gap-3">
            <p className="text-xs" style={{ color: '#A5A3AE' }}>Terakhir sync: {lastSync}</p>
          ...
          {PLATFORMS.map(p => (
            <div key={p.id} className="bg-white rounded-2xl p-5" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-bold" style={{ color: '#433C50' }}>{p.name}</p>
                  <p className="text-xs" style={{ color: '#4CAF50' }}>● Terhubung</p>
                </div>
              </div>
              <div className="flex justify-between text-xs">
                <span style={{ color: '#A5A3AE' }}>Produk Sinkron</span>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari produk atau SKU..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE8F5', backgroundColor: '#F8F7FC' }}>
                  <th className="px-4 py-3 text-left text-xs font-semibold" style={{ color: '#A5A3AE' }}>SKU</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold" style={{ color: '#A5A3AE' }}>Produk</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#433C50' }}>Harga ERP</th>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold" style={{ color: '#433C50' }}>TikTok</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold" style={{ color: '#433C50' }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(p => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-mono text-xs font-semibold" style={{ color: C }}>{p.sku}</td>
                      <td className="px-4 py-3 font-medium" style={{ color: '#433C50' }}>{p.name}</td>
                      <td className="px-4 py-3 text-right font-bold text-xs" style={{ color: '#433C50' }}>{fmt(p.erp_price)}</td>
                      <td className="px-4 py-3 text-right text-xs" style={{ color: p.shopee_price !== p.erp_price && p.shopee_price ? '#FF9800' : '#433C50' }}>
                      ...
                      <td className="px-4 py-3 text-right text-xs" style={{ color: p.tokopedia_price !== p.erp_price && p.tokopedia_price ? '#FF9800' : '#433C50' }}>
                      ...
                      <td className="px-4 py-3 text-right text-xs" style={{ color: p.tiktok_price !== p.erp_price && p.tiktok_price ? '#FF9800' : '#433C50' }}>
