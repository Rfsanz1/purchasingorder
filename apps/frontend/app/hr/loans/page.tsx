    ...
    <AppShell {...HR_CONFIG} navItems={HR_NAV} activeHref="/hr/loans">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Pinjaman Karyawan</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola pinjaman dan cicilan karyawan yang dipotong dari gaji</p>
          </div>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium" style={{ color: '#433C50' }}>{item.employee}</td>
                      <td className="px-4 py-3 font-bold" style={{ color: '#433C50' }}>{fmt(item.amount)}</td>
                      <td className="px-4 py-3 text-xs text-center" style={{ color: '#6D6777' }}>{item.tenor} bln</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs" style={{ color: '#6D6777' }}>{item.paid}x</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-xs text-center" style={{ color: '#433C50' }}>{item.remaining}x</td>
                      ...
                      <td className="px-4 py-3 text-xs max-w-[120px] truncate" style={{ color: '#6D6777' }}>{item.purpose}</td>
                      <td className="px-4 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Ajukan Pinjaman Karyawan</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                {form.amount && form.tenor && (
                  <div className="rounded-xl p-3" style={{ backgroundColor: `${C}08`, border: `1px solid ${C}30` }}>
                    <p className="text-xs" style={{ color: '#6D6777' }}>Cicilan per bulan:</p>
