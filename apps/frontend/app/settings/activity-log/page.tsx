    ...
    <AppShell {...SETTINGS_CONFIG} navItems={SETTINGS_NAV} activeHref="/settings/activity-log">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Activity Log</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Riwayat aktivitas semua pengguna sistem untuk audit trail</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4 flex-wrap" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari user atau deskripsi..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            <select className="rounded-lg px-3 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={actionFilter} onChange={e => setActionFilter(e.target.value)}>
              <option value="">Semua Aksi</option>
              {actions.map(a => <option key={a} value={a}>{a}</option>)}
            </select>
            <select className="rounded-lg px-3 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={moduleFilter} onChange={e => setModuleFilter(e.target.value)}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(log => {
                  ...
                  return (
                    <tr key={log.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      <td className="px-6 py-3 text-xs whitespace-nowrap" style={{ color: '#A5A3AE' }}>{log.timestamp}</td>
                      <td className="px-6 py-3 font-medium text-xs" style={{ color: '#433C50' }}>{log.user}</td>
                      ...
                      <td className="px-6 py-3 text-xs" style={{ color: '#6D6777' }}>{log.module}</td>
                      <td className="px-6 py-3 text-xs max-w-xs truncate" style={{ color: '#433C50' }}>{log.description}</td>
                      <td className="px-6 py-3 text-xs font-mono" style={{ color: '#A5A3AE' }}>{log.ip}</td>
                    </tr>
                  );
                })}
              </tbody>
