    ...
    <AppShell {...SETTINGS_CONFIG} navItems={SETTINGS_NAV} activeHref="/settings/roles">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Role & Permission</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola role dan hak akses per modul sistem</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="space-y-2">
            <p className="text-xs font-semibold px-1" style={{ color: '#A5A3AE' }}>DAFTAR ROLE</p>
            {roles.map(role => (
              <div key={role.id} onClick={() => selectRole(role)} className="bg-white rounded-xl p-4 cursor-pointer transition-all hover:shadow-md" style={{ border: `2px solid ${selected.id === role.id ? role.color : '#EDE8F5'}` }}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm truncate" style={{ color: '#433C50' }}>{role.name}</p>
                    <p className="text-xs truncate" style={{ color: '#A5A3AE' }}>{role.users} user · {role.desc.substring(0, 30)}...</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="col-span-2 bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm" style={{ color: '#433C50' }}>{selected.name}</h3>
                  <p className="text-xs" style={{ color: '#A5A3AE' }}>{selected.desc}</p>
                </div>
              </div>
            ...
            {selected.name === 'Administrator' ? (
              <div className="p-6 text-center">
                ...
                <p className="font-semibold" style={{ color: '#433C50' }}>Role Administrator</p>
                <p className="text-sm mt-1" style={{ color: '#A5A3AE' }}>Administrator memiliki akses penuh ke semua modul dan tidak dapat dibatasi.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr style={{ borderBottom: '1px solid #EDE8F5', backgroundColor: '#F8F7FC' }}>
                      <th className="px-4 py-3 text-left font-semibold" style={{ color: '#A5A3AE' }}>Modul</th>
                      {PERMS.map(p => (
                        <th key={p} className="px-3 py-3 text-center font-semibold" style={{ color: '#A5A3AE' }}>{p}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {MODULES_LIST.map(mod => (
                      <tr key={mod} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                        <td className="px-4 py-2.5 font-medium" style={{ color: '#433C50' }}>{mod}</td>
                        {PERMS.map(perm => (
                          <td key={perm} className="px-3 py-2.5 text-center">
                            <button onClick={() => togglePerm(mod, perm)} className="h-5 w-5 rounded flex items-center justify-center mx-auto transition-colors" style={{ backgroundColor: hasPerm(mod, perm) ? C : '#F5F5F9', border: `1.5px solid ${hasPerm(mod, perm) ? C : '#D1D5DB'}` }}>
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-sm mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Tambah Role Baru</h2>
                <button onClick={() => setShowForm(false)} style={{ color: '#A5A3AE' }}><X className="h-5 w-5" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Nama Role *</label>
                  <input className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Nama role..." value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = '#EDE8F5'} />
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Deskripsi</label>
                  <input className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Deskripsi role..." value={form.desc} onChange={e => setForm(f => ({ ...f, desc: e.target.value }))} onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = '#EDE8F5'} />
                </div>
