    ...
    <AppShell {...MANUFACTURING_CONFIG} navItems={MANUFACTURING_NAV} activeHref="/manufacturing/scrap">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Scrap Management</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Catat dan kelola barang hasil reject dan scrap produksi</p>
          </div>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-xs font-mono" style={{ color: '#6D6777' }}>{item.lot ?? '-'}</td>
                    <td className="px-4 py-3 font-bold" style={{ color: '#EA5455' }}>{item.qty} {item.uom}</td>
                    <td className="px-4 py-3 text-xs max-w-xs truncate" style={{ color: '#6D6777' }}>{item.reason}</td>
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: '#433C50' }}>{item.operator}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-lg mx-4 overflow-y-auto" style={{ maxHeight: '90vh', boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Input Scrap</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Alasan Scrap *</label>
                  <textarea className="w-full rounded-lg px-4 py-2.5 text-sm resize-none" rows={3} style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Deskripsikan alasan scrap..." value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))} />
                </div>
