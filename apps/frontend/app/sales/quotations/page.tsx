    ...
    <AppShell {...SALES_CONFIG} navItems={SALES_NAV} activeHref="/sales/quotations">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Quotation</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Buat dan kelola penawaran harga ke pelanggan</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari pelanggan atau nomor..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          ...
          {loading ? (
            <div className="p-8 text-center text-sm" style={{ color: '#A5A3AE' }}>Memuat data...</div>
          ) : items.length === 0 ? (
            <div className="p-12 text-center">
              <FileText className="h-12 w-12 mx-auto mb-3 opacity-30" style={{ color: C }} />
              <p className="text-sm font-medium" style={{ color: '#A5A3AE' }}>Belum ada quotation</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-3 font-semibold text-xs" style={{ color: C }}>QUO-{String(i + 1).padStart(4, '0')}</td>
                        <td className="px-6 py-3" style={{ color: '#433C50' }}>{item.customerName ?? '-'}</td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{item.createdAt ? new Date(item.createdAt).toLocaleDateString('id-ID') : '-'}</td>
                        <td className="px-6 py-3 font-semibold" style={{ color: '#433C50' }}>{Number(item.totalAmount ?? 0).toLocaleString('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 })}</td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{item.paymentTerm ?? 'Net 30'}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-3xl mx-4 overflow-hidden" style={{ maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buat Quotation Baru</h2>
                <button onClick={() => setShowForm(false)} style={{ color: '#A5A3AE' }}><X className="h-5 w-5" /></button>
              </div>
              <div className="p-6 space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Pelanggan *</label>
                    <input className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Nama pelanggan..." value={form.customer} onChange={e => setForm(f => ({ ...f, customer: e.target.value }))} onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = '#EDE8F5'} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Payment Term</label>
                    <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.payment_term} onChange={e => setForm(f => ({ ...f, payment_term: e.target.value }))}>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-xs font-semibold" style={{ color: '#433C50' }}>Produk & Layanan</label>
                  ...
                  <div className="rounded-xl overflow-hidden" style={{ border: '1.5px solid #EDE8F5' }}>
                    <table className="w-full text-xs">
                      <thead style={{ backgroundColor: '#F8F7FC' }}>
                        <tr>
                          {['Produk', 'Qty', 'Harga', 'Diskon (%)', 'Pajak (%)', 'Subtotal', ''].map(h => (
                            <th key={h} className="px-3 py-2.5 text-left font-semibold" style={{ color: '#A5A3AE' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {form.lines.map((line, i) => (
                          <tr key={i} style={{ borderTop: '1px solid #EDE8F5' }}>
                            ...
                            <td className="px-3 py-2 font-semibold" style={{ color: '#433C50' }}>
                  ...
                  <div className="flex justify-end mt-3 space-y-1 text-xs">
                    <div className="w-56 space-y-1">
                      <div className="flex justify-between" style={{ color: '#6D6777' }}><span>Subtotal</span><span>{subtotal.toLocaleString('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 })}</span></div>
                      <div className="flex justify-between" style={{ color: '#6D6777' }}><span>PPN</span><span>{tax.toLocaleString('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 })}</span></div>
                      <div className="flex justify-between font-bold text-sm pt-1" style={{ color: '#433C50', borderTop: '1px solid #EDE8F5' }}><span>Total</span><span>{total.toLocaleString('id-ID', { style: 'currency', currency: 'IDR', maximumFractionDigits: 0 })}</span></div>
                    </div>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Catatan Internal</label>
                  <textarea className="w-full rounded-lg px-4 py-2.5 text-sm resize-none" rows={3} style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Catatan untuk tim internal..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
                </div>
