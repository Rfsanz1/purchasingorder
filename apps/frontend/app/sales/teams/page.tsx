    ...
    <AppShell {...SALES_CONFIG} navItems={SALES_NAV} activeHref="/sales/teams">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Sales Team</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola tim penjualan dan pantau performa masing-masing anggota</p>
          </div>
            ...
            return (
              <div key={team.id} onClick={() => setSelected(team)} className="bg-white rounded-2xl p-5 cursor-pointer transition-all hover:shadow-md" style={{ border: `2px solid ${isSelected ? C : '#EDE8F5'}`, boxShadow: isSelected ? `0 4px 16px ${C}20` : '0 1px 4px rgba(47,43,61,.06)' }}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-bold" style={{ color: '#433C50' }}>{team.name}</p>
                    <p className="text-xs mt-0.5" style={{ color: '#A5A3AE' }}>Leader: {team.leader} • {team.members.length} anggota</p>
                  </div>
        ...
        {selected && (
          <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
            <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
              <h3 className="font-semibold text-sm" style={{ color: '#433C50' }}>Performa Anggota — {selected.name}</h3>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {selected.members.map((m, i) => {
                    ...
                    const isTop = i === 0;
                    return (
                      <tr key={m.name} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium" style={{ color: '#433C50' }}>{m.name}</span>
                          </div>
                        </td>
                        <td className="px-6 py-3" style={{ color: '#6D6777' }}>{fmt(m.target)}</td>
                        <td className="px-6 py-3 font-semibold" style={{ color: pct >= 100 ? '#4CAF50' : '#433C50' }}>{fmt(m.achieved)}</td>
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="px-6 py-3 text-center font-semibold" style={{ color: '#433C50' }}>{m.leads}</td>
                        <td className="px-6 py-3 text-center font-semibold" style={{ color: '#433C50' }}>{m.deals}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Tambah Sales Team</h2>
              ...
              <div className="p-6 space-y-4">
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder={f.placeholder} value={(form as any)[f.key]} onChange={e => setForm(f2 => ({ ...f2, [f.key]: e.target.value }))} onFocus={e => e.target.style.borderColor = C} onBlur={e => e.target.style.borderColor = '#EDE8F5'} />
                  </div>
                ))}
