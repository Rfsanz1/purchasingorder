    ...
    <AppShell {...INVOICE_CONFIG} navItems={INVOICE_NAV} activeHref="/invoice/down-payment">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Down Payment (DP)</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Kelola uang muka penjualan dan pengurangan dari invoice akhir</p>
          </div>
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="relative flex-1 max-w-sm">
              ...
              <input className="w-full rounded-lg pl-9 pr-4 py-2 text-sm" style={{ border: '1px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Cari pelanggan atau SO..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE8F5', backgroundColor: '#F8F7FC' }}>
                  {['No. DP', 'Sales Order', 'Pelanggan', 'Total Order', 'DP %', 'Nilai DP', 'Dibayar', 'Sisa Tagihan', 'Jatuh Tempo', 'Status', 'Aksi'].map(h => (
                    <th key={h} className="px-3 py-3 text-left text-xs font-semibold whitespace-nowrap" style={{ color: '#A5A3AE' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-xs font-semibold" style={{ color: '#6D6777' }}>{item.so}</td>
                      <td className="px-3 py-3 font-medium" style={{ color: '#433C50' }}>{item.customer}</td>
                      <td className="px-3 py-3 text-xs" style={{ color: '#433C50' }}>{fmt(item.total_order)}</td>
                      <td className="px-3 py-3 font-bold text-sm text-center" style={{ color: C }}>{item.dp_pct}%</td>
                      <td className="px-3 py-3 font-semibold text-xs" style={{ color: '#433C50' }}>{fmt(item.dp_amount)}</td>
                      <td className="px-3 py-3 font-semibold text-xs" style={{ color: '#4CAF50' }}>{fmt(item.paid_amount)}</td>
                      <td className="px-3 py-3 font-semibold text-xs" style={{ color: '#433C50' }}>{fmt(item.remaining)}</td>
                      <td className="px-3 py-3 text-xs" style={{ color: '#6D6777' }}>{new Date(item.due_date).toLocaleDateString('id-ID')}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buat Invoice Down Payment</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Persentase DP (%)</label>
                  <div className="flex gap-2">
                    {['10', '20', '30', '50'].map(pct => (
                      <button key={pct} onClick={() => setForm(f => ({ ...f, dp_pct: pct }))} className="flex-1 py-2 rounded-lg text-sm font-semibold transition" style={{ backgroundColor: form.dp_pct === pct ? C : '#F8F7FC', color: form.dp_pct === pct ? '#FFFFFF' : '#6D6777', border: `1.5px solid ${form.dp_pct === pct ? C : '#EDE8F5'}` }}>
                        {pct}%
                      </button>
                    ))}
                    <input type="number" className="flex-1 py-2 rounded-lg text-sm text-center" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} placeholder="Other" value={form.dp_pct} onChange={e => setForm(f => ({ ...f, dp_pct: e.target.value }))} />
                  </div>
                </div>
                {dp_amount > 0 && (
                  <div className="rounded-xl p-3" style={{ backgroundColor: `${C}08`, border: `1px solid ${C}30` }}>
                    <p className="text-xs" style={{ color: '#6D6777' }}>Nilai Down Payment:</p>
