    ...
    <AppShell {...INVOICE_CONFIG} navItems={INVOICE_NAV} activeHref="/invoice/aging">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Aging Invoice Report</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Invoice yang melewati jatuh tempo pembayaran</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: 'Rata-rata Hari', value: `${Math.round(SAMPLE.reduce((s, i) => s + i.days_overdue, 0) / SAMPLE.length)} hr`, color: '#433C50' },
          ].map(s => (
        ...
        <div className="bg-white rounded-2xl" style={{ border: '1.5px solid #EDE8F5', boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <h3 className="font-semibold text-sm" style={{ color: '#433C50' }}>Daftar Invoice Jatuh Tempo</h3>
          </div>
                ...
                {SAMPLE.sort((a, b) => b.days_overdue - a.days_overdue).map(item => {
                  ...
                  return (
                    <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-3 font-bold" style={{ color: '#433C50' }}>{fmt(item.amount)}</td>
                      <td className="px-6 py-3 text-xs" style={{ color: '#6D6777' }}>{new Date(item.due_date).toLocaleDateString('id-ID')}</td>
