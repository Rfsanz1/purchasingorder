    ...
    <AppShell {...INVOICE_CONFIG} navItems={INVOICE_NAV} activeHref="/invoice/list">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <div><h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Daftar Invoice</h1><p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Semua tagihan pelanggan</p></div>
          ...
          <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
            <div className="flex gap-1 p-1 rounded-xl" style={{ backgroundColor: '#F5F2FB' }}>
              {['Semua', 'Draft', 'Dikirim', 'Lunas', 'Jatuh Tempo'].map(tab => (
                <button key={tab} className="px-3 py-1.5 rounded-lg text-xs font-semibold transition" style={{ backgroundColor: tab === 'Semua' ? 'white' : 'transparent', color: tab === 'Semua' ? '#433C50' : '#A5A3AE' }}>{tab}</button>
              ))}
            </div>
