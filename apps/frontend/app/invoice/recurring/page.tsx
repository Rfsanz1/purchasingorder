    ...
    <AppShell {...INVOICE_CONFIG} navItems={INVOICE_NAV} activeHref="/invoice/recurring">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold" style={{ color: '#433C50' }}>Recurring Invoice</h1>
            <p className="text-sm mt-0.5" style={{ color: '#A5A3AE' }}>Invoice otomatis berulang untuk pelanggan reguler</p>
          </div>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #F5F5F9' }} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-semibold" style={{ color: '#433C50' }}>{item.customer}</td>
                    ...
                    <td className="px-6 py-4" style={{ color: '#6D6777' }}>{new Date(item.next_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</td>
                    <td className="px-6 py-4 text-center font-semibold" style={{ color: '#433C50' }}>{item.total_sent}x</td>
                    <td className="px-6 py-4">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl w-full max-w-md mx-4" style={{ boxShadow: '0 20px 60px rgba(0,0,0,.15)' }}>
              <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EDE8F5' }}>
                <h2 className="font-bold" style={{ color: '#433C50' }}>Buat Recurring Invoice</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: '#433C50' }}>Interval</label>
                  <select className="w-full rounded-lg px-4 py-2.5 text-sm" style={{ border: '1.5px solid #EDE8F5', color: '#433C50', outline: 'none' }} value={form.interval} onChange={e => setForm(f => ({ ...f, interval: e.target.value }))}>
