...
// ── SALES ──────────────────────────────────────────────────────────────
Route::get('/erp/sales', fn() => view('erp.generic-module', ['title'=>'Sales','description'=>'Kelola order penjualan, konfirmasi, dan pengiriman ke pelanggan.','features'=>['Sales Order','Konfirmasi Otomatis','Harga Khusus','Multi Warehouse','Invoice Otomatis','Portal Pelanggan']]));
Route::get('/erp/subscriptions', fn() => view('erp.generic-module', ['title'=>'Subscriptions','description'=>'Kelola langganan berkala dan pembayaran berulang secara otomatis.','features'=>['Recurring Invoice','Trial Period','Upsell/Downsell','Churn Analysis','Auto Renewal','MRR Dashboard']]));
Route::get('/erp/rental', fn() => view('erp.generic-module', ['title'=>'Rental','description'=>'Kelola persewaan produk, jadwal pengembalian, dan deposit.','features'=>['Rental Order','Jadwal Ketersediaan','Deposit Management','Late Return Alert','Invoice Otomatis','Pelacakan Produk']]));

// ── FINANCE ────────────────────────────────────────────────────────────
Route::get('/erp/accounting', fn() => view('erp.generic-module', ['title'=>'Accounting','description'=>'Sistem akuntansi lengkap terintegrasi dengan semua modul ERP.','features'=>['Double Entry','Multi-Currency','Bank Sync','Auto Reconciliation','Financial Reports','Tax Compliance']]));
Route::get('/erp/invoicing', fn() => view('erp.generic-module', ['title'=>'Invoicing','description'=>'Buat dan kelola invoice pelanggan dengan mudah dan cepat.','features'=>['Invoice Template','Email Otomatis','Payment Link','Partial Payment','Credit Note','Invoice Portal']]));
Route::get('/erp/expenses', fn() => view('erp.crud', ['module'=>'expenses','title'=>'Expenses','description'=>'Pengeluaran karyawan, reimburse, dan laporan expense.','addLabel'=>'Tambah Expense','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Submitted','Approved','Refused','Posted'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'expense_date','label'=>'Tanggal','type'=>'date'],['name'=>'category','label'=>'Kategori','type'=>'text'],['name'=>'description','label'=>'Deskripsi','type'=>'text'],['name'=>'total','label'=>'Total','type'=>'number'],['name'=>'payment_mode','label'=>'Pembayaran','type'=>'select','options'=>['Own Money','Company Money']],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Submitted','Approved','Refused','Posted']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'expense_date','label'=>'Tanggal'],['name'=>'category','label'=>'Kategori'],['name'=>'total','label'=>'Total','format'=>'currency'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/sign', fn() => view('erp.generic-module', ['title'=>'Sign — Tanda Tangan Digital','description'=>'Buat, kirim, dan kelola dokumen dengan tanda tangan digital yang sah secara hukum.','features'=>['Template Dokumen','Kirim via Email','Multi Signer','Audit Trail','Certificate','Integrasi PDF']]));
Route::get('/erp/multi-currency', fn() => view('erp.generic-module', ['title'=>'Multi Currency','description'=>'Kelola transaksi dalam berbagai mata uang dengan kurs otomatis.','features'=>['Live Exchange Rate','Multi-Currency Invoice','Gain/Loss Otomatis','Currency Rounding','Bank Statement Import','FX Report']]));
Route::get('/erp/main-cash', fn() => view('erp.generic-module', ['title'=>'Kas Utama','description'=>'Kelola saldo kas utama perusahaan dan transaksi harian.','features'=>['Saldo Real-time','Mutasi Kas','Cash Register','Kasbon','Opname Kas','Laporan Kas']]));
Route::get('/erp/petty-cash', fn() => view('erp.generic-module', ['title'=>'Kas Kecil','description'=>'Kelola kas kecil (petty cash) untuk pengeluaran operasional harian.','features'=>['Pengisian Kas','Pengeluaran','Saldo Minimum Alert','Laporan Petty Cash','Approval','Rekap Bulanan']]));
Route::get('/erp/electronic-cash', fn() => view('erp.generic-module', ['title'=>'Kas Elektronik','description'=>'Kelola saldo e-wallet, GoPay, OVO, dan kas digital lainnya.','features'=>['Multi E-Wallet','Top Up & Withdraw','Rekonsiliasi','QR Code Payment','Notifikasi Realtime','Laporan']]));
Route::get('/erp/giro', fn() => view('erp.generic-module', ['title'=>'Giro','description'=>'Kelola giro masuk dan giro keluar dengan tracking jatuh tempo.','features'=>['Giro Masuk','Giro Keluar','Jatuh Tempo Alert','Pencairan','Bank Clearing','Laporan Giro']]));
Route::get('/erp/opening-balance', fn() => view('erp.generic-module', ['title'=>'Saldo Awal','description'=>'Input saldo awal akun untuk periode pertama pembukuan.','features'=>['Saldo Awal Akun','Import dari Excel','Verifikasi','Periode Lock','Audit Trail','Rollback']]));
Route::get('/erp/accounting-period', fn() => view('erp.generic-module', ['title'=>'Periode Akuntansi','description'=>'Kelola periode akuntansi dan penutupan buku tahunan.','features'=>['Buka/Tutup Periode','Jurnal Penutup','Carry Forward','Audit Lock','Multi Periode','Laporan']]));

// ── MANUFACTURING ──────────────────────────────────────────────────────
Route::get('/erp/manufacturing', fn() => view('erp.generic-module', ['title'=>'Manufacturing','description'=>'Kelola proses produksi, Bill of Materials, dan work orders.','features'=>['Bill of Materials','Work Orders','Work Center','Production Planning','By-Products','Cost Analysis']]));
Route::get('/erp/plm', fn() => view('erp.generic-module', ['title'=>'PLM — Product Lifecycle','description'=>'Kelola siklus hidup produk dari desain hingga produksi massal.','features'=>['Engineering Change Order','Version Control','BOM Revision','Approval Workflow','Dokumen Teknis','Integrasi Manufaktur']]));
Route::get('/erp/quality', fn() => view('erp.generic-module', ['title'=>'Quality Control','description'=>'Kelola kontrol kualitas produk di setiap tahap produksi dan penerimaan.','features'=>['Quality Points','Control Plans','Quality Alerts','Failure Analysis','FMEA','Integrasi Inventory']]));
Route::get('/erp/maintenance', fn() => view('erp.generic-module', ['title'=>'Maintenance','description'=>'Kelola perawatan mesin, jadwal preventif, dan corrective maintenance.','features'=>['Preventive Maintenance','Corrective Maintenance','Work Order Mesin','MTBF/MTTR','Spare Parts','Cost Analysis']]));
Route::get('/erp/bom', fn() => view('erp.crud', ['module'=>'boms','title'=>'Bill of Materials','description'=>'Daftar bahan baku dan komponen untuk setiap produk jadi.','addLabel'=>'Tambah BOM','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Active','Obsolete'],'formFields'=>[['name'=>'product','label'=>'Produk Jadi','type'=>'text','required'=>true],['name'=>'quantity','label'=>'Qty','type'=>'number'],['name'=>'bom_type','label'=>'Tipe BOM','type'=>'select','options'=>['Manufacture','Kit','Subcontracting']],['name'=>'components','label'=>'Komponen','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Active','Obsolete']]],'tableFields'=>[['name'=>'product','label'=>'Produk'],['name'=>'quantity','label'=>'Qty'],['name'=>'bom_type','label'=>'Tipe'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/work-orders', fn() => view('erp.crud', ['module'=>'work-orders','title'=>'Work Orders','description'=>'Perintah kerja produksi di setiap work center.','addLabel'=>'Tambah Work Order','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','In Progress','Done','Cancelled'],'formFields'=>[['name'=>'production_id','label'=>'Manufacturing Order','type'=>'text','required'=>true],['name'=>'work_center','label'=>'Work Center','type'=>'text'],['name'=>'duration','label'=>'Durasi (menit)','type'=>'number'],['name'=>'employee','label'=>'Operator','type'=>'text'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','In Progress','Done','Cancelled']]],'tableFields'=>[['name'=>'production_id','label'=>'MO'],['name'=>'work_center','label'=>'Work Center'],['name'=>'duration','label'=>'Durasi'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/scraps', fn() => view('erp.crud', ['module'=>'scraps','title'=>'Scraps — Sisa Produksi','description'=>'Catat bahan baku dan produk yang menjadi scrap/sisa.','addLabel'=>'Tambah Scrap','formFields'=>[['name'=>'product','label'=>'Produk','type'=>'text','required'=>true],['name'=>'quantity','label'=>'Qty','type'=>'number'],['name'=>'location','label'=>'Lokasi Asal','type'=>'text'],['name'=>'scrap_location','label'=>'Lokasi Scrap','type'=>'text'],['name'=>'reason','label'=>'Alasan','type'=>'textarea']],'tableFields'=>[['name'=>'product','label'=>'Produk'],['name'=>'quantity','label'=>'Qty'],['name'=>'location','label'=>'Dari'],['name'=>'reason','label'=>'Alasan']]]));
Route::get('/erp/lot-serial', fn() => view('erp.crud', ['module'=>'lot-serials','title'=>'Lot & Serial Number','description'=>'Lacak produk berdasarkan lot atau nomor seri unik.','addLabel'=>'Tambah Lot/SN','formFields'=>[['name'=>'product','label'=>'Produk','type'=>'text','required'=>true],['name'=>'lot_number','label'=>'Lot/Serial No','type'=>'text','required'=>true],['name'=>'expiry_date','label'=>'Exp Date','type'=>'date'],['name'=>'origin','label'=>'Sumber','type'=>'text'],['name'=>'notes','label'=>'Catatan','type'=>'textarea']],'tableFields'=>[['name'=>'lot_number','label'=>'Lot/SN'],['name'=>'product','label'=>'Produk'],['name'=>'expiry_date','label'=>'Exp Date']]]));

// ── PURCHASE ───────────────────────────────────────────────────────────
Route::get('/erp/rfq', fn() => view('erp.crud', ['module'=>'rfqs','title'=>'Request for Quotation (RFQ)','description'=>'Permintaan penawaran harga kepada supplier sebelum PO dibuat.','addLabel'=>'Buat RFQ','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Sent','Purchase Order','Cancelled'],'formFields'=>[['name'=>'supplier','label'=>'Supplier','type'=>'text','required'=>true],['name'=>'rfq_date','label'=>'Tanggal RFQ','type'=>'date'],['name'=>'products','label'=>'Produk','type'=>'textarea'],['name'=>'deadline','label'=>'Deadline','type'=>'date'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Sent','Purchase Order','Cancelled']]],'tableFields'=>[['name'=>'supplier','label'=>'Supplier'],['name'=>'rfq_date','label'=>'Tanggal'],['name'=>'deadline','label'=>'Deadline'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/vendor-pricelist', fn() => view('erp.crud', ['module'=>'vendor-pricelists','title'=>'Vendor Pricelist','description'=>'Daftar harga pembelian dari setiap supplier per produk.','addLabel'=>'Tambah Harga Vendor','formFields'=>[['name'=>'supplier','label'=>'Supplier','type'=>'text','required'=>true],['name'=>'product','label'=>'Produk','type'=>'text','required'=>true],['name'=>'price','label'=>'Harga','type'=>'number','step'=>'0.01'],['name'=>'min_qty','label'=>'Min Qty','type'=>'number'],['name'=>'currency','label'=>'Mata Uang','type'=>'text'],['name'=>'start_date','label'=>'Berlaku Dari','type'=>'date'],['name'=>'end_date','label'=>'Berlaku Sampai','type'=>'date']],'tableFields'=>[['name'=>'supplier','label'=>'Supplier'],['name'=>'product','label'=>'Produk'],['name'=>'price','label'=>'Harga','format'=>'currency'],['name'=>'min_qty','label'=>'Min Qty']]]));

// ── HUMAN RESOURCES ────────────────────────────────────────────────────
Route::get('/erp/recruitment', fn() => view('erp.crud', ['module'=>'recruitments','title'=>'Recruitment','description'=>'Kelola proses rekrutmen dari lowongan hingga onboarding karyawan baru.','addLabel'=>'Buat Lowongan','statusField'=>'stage','filterField'=>'stage','filterOptions'=>['New','Qualification','Interview','Offer','Contract Signed','Refused'],'formFields'=>[['name'=>'job_position','label'=>'Posisi','type'=>'text','required'=>true],['name'=>'department','label'=>'Departemen','type'=>'text'],['name'=>'responsible','label'=>'Rekruter','type'=>'text'],['name'=>'expected_date','label'=>'Target Tanggal','type'=>'date'],['name'=>'applicant_name','label'=>'Nama Pelamar','type'=>'text'],['name'=>'email','label'=>'Email','type'=>'email'],['name'=>'mobile','label'=>'No HP','type'=>'text'],['name'=>'stage','label'=>'Tahap','type'=>'select','options'=>['New','Qualification','Interview','Offer','Contract Signed','Refused']],['name'=>'notes','label'=>'Catatan','type'=>'textarea']],'tableFields'=>[['name'=>'job_position','label'=>'Posisi'],['name'=>'applicant_name','label'=>'Pelamar'],['name'=>'department','label'=>'Dept'],['name'=>'stage','label'=>'Tahap','badge'=>true]]]));
Route::get('/erp/time-off', fn() => view('erp.crud', ['module'=>'time-offs','title'=>'Time Off / Cuti','description'=>'Kelola pengajuan cuti, izin, dan alokasi hari libur karyawan.','addLabel'=>'Ajukan Cuti','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Confirmed','Approved','Refused','Cancelled'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'leave_type','label'=>'Jenis Cuti','type'=>'select','options'=>['Cuti Tahunan','Cuti Sakit','Izin','Cuti Melahirkan','Cuti Ayah','Cuti Khusus']],['name'=>'date_from','label'=>'Dari','type'=>'date'],['name'=>'date_to','label'=>'Sampai','type'=>'date'],['name'=>'duration','label'=>'Durasi (hari)','type'=>'number'],['name'=>'reason','label'=>'Alasan','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Confirmed','Approved','Refused','Cancelled']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'leave_type','label'=>'Jenis'],['name'=>'date_from','label'=>'Dari'],['name'=>'date_to','label'=>'Sampai'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/appraisals', fn() => view('erp.crud', ['module'=>'appraisals','title'=>'Appraisals — Penilaian Kinerja','description'=>'Evaluasi kinerja karyawan secara berkala dengan sistem scoring terstruktur.','addLabel'=>'Buat Penilaian','statusField'=>'status','filterField'=>'status','filterOptions'=>['New','Confirmed','Done'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'reviewer','label'=>'Penilai','type'=>'text'],['name'=>'date','label'=>'Tanggal Penilaian','type'=>'date'],['name'=>'period','label'=>'Periode','type'=>'text'],['name'=>'score','label'=>'Nilai (1-10)','type'=>'number'],['name'=>'strengths','label'=>'Kelebihan','type'=>'textarea'],['name'=>'improvements','label'=>'Area Perbaikan','type'=>'textarea'],['name'=>'goals','label'=>'Target Berikutnya','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['New','Confirmed','Done']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'reviewer','label'=>'Penilai'],['name'=>'period','label'=>'Periode'],['name'=>'score','label'=>'Nilai'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/lunch', fn() => view('erp.crud', ['module'=>'lunch-orders','title'=>'Lunch — Makan Siang','description'=>'Kelola pemesanan makan siang karyawan dan subsidi perusahaan.','addLabel'=>'Pesan Makan','statusField'=>'status','filterField'=>'status','filterOptions'=>['Ordered','Received','Cancelled'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'menu','label'=>'Menu','type'=>'text'],['name'=>'supplier','label'=>'Vendor Makanan','type'=>'text'],['name'=>'price','label'=>'Harga','type'=>'number'],['name'=>'date','label'=>'Tanggal','type'=>'date'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Ordered','Received','Cancelled']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'menu','label'=>'Menu'],['name'=>'date','label'=>'Tanggal'],['name'=>'price','label'=>'Harga','format'=>'currency'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/referrals', fn() => view('erp.generic-module', ['title'=>'Referrals — Referral Karyawan','description'=>'Program referral karyawan untuk merekrut kandidat melalui jaringan internal.','features'=>['Referral Points','Reward Sistem','Tracking Aplikasi','Leaderboard','Notifikasi','Dashboard Karyawan']]));
Route::get('/erp/employee-contracts', fn() => view('erp.crud', ['module'=>'employee-contracts','title'=>'Kontrak Kerja','description'=>'Kelola kontrak kerja karyawan beserta durasi dan kondisi perjanjian.','addLabel'=>'Buat Kontrak','statusField'=>'status','filterField'=>'status','filterOptions'=>['New','Active','Expired','Cancelled'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'job','label'=>'Jabatan','type'=>'text'],['name'=>'wage','label'=>'Gaji Pokok','type'=>'number'],['name'=>'date_start','label'=>'Mulai','type'=>'date'],['name'=>'date_end','label'=>'Berakhir','type'=>'date'],['name'=>'contract_type','label'=>'Tipe','type'=>'select','options'=>['PKWT','PKWTT','Freelance','Magang']],['name'=>'status','label'=>'Status','type'=>'select','options'=>['New','Active','Expired','Cancelled']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'job','label'=>'Jabatan'],['name'=>'wage','label'=>'Gaji','format'=>'currency'],['name'=>'date_end','label'=>'Berakhir'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/salary-structure', fn() => view('erp.generic-module', ['title'=>'Struktur Gaji','description'=>'Kelola komponen gaji, tunjangan, potongan, dan rumus penghitungan payroll.','features'=>['Gaji Pokok','Tunjangan Jabatan','Tunjangan Transport','BPJS Kesehatan','BPJS Ketenagakerjaan','PPh 21','Potongan','Bonus','Lembur']]));

// ── MARKETING ──────────────────────────────────────────────────────────
Route::get('/erp/email-marketing', fn() => view('erp.generic-module', ['title'=>'Email Marketing','description'=>'Buat dan kirim kampanye email ke pelanggan dengan tracking buka dan klik.','features'=>['Template Drag & Drop','Segmentasi Kontak','A/B Testing','Open Rate Tracking','Click Rate','Unsubscribe Handling','Schedule Kirim']]));
Route::get('/erp/sms-marketing', fn() => view('erp.generic-module', ['title'=>'SMS Marketing','description'=>'Kirim SMS blast ke pelanggan langsung dari sistem ERP.','features'=>['Template SMS','Segmentasi','Delivery Report','Fonnte Integration','Schedule','Cost Tracking']]));
Route::get('/erp/social-marketing', fn() => view('erp.generic-module', ['title'=>'Social Marketing','description'=>'Kelola postingan media sosial dari satu tempat, jadwalkan dan pantau engagement.','features'=>['Multi Platform','Instagram/Facebook','Schedule Post','Analytics','UTM Tracking','Content Calendar']]));
Route::get('/erp/events', fn() => view('erp.crud', ['module'=>'events','title'=>'Events','description'=>'Kelola event, seminar, workshop, dan pendaftaran peserta.','addLabel'=>'Buat Event','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Published','Ended','Cancelled'],'formFields'=>[['name'=>'event_name','label'=>'Nama Event','type'=>'text','required'=>true],['name'=>'date_begin','label'=>'Mulai','type'=>'date'],['name'=>'date_end','label'=>'Selesai','type'=>'date'],['name'=>'location','label'=>'Lokasi','type'=>'text'],['name'=>'organizer','label'=>'Penyelenggara','type'=>'text'],['name'=>'seats_max','label'=>'Kapasitas','type'=>'number'],['name'=>'website_published','label'=>'Tampil di Website','type'=>'select','options'=>['Ya','Tidak']],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Published','Ended','Cancelled']]],'tableFields'=>[['name'=>'event_name','label'=>'Event'],['name'=>'date_begin','label'=>'Mulai'],['name'=>'location','label'=>'Lokasi'],['name'=>'seats_max','label'=>'Kapasitas'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/surveys', fn() => view('erp.crud', ['module'=>'surveys','title'=>'Surveys — Kuesioner','description'=>'Buat dan distribusikan survei untuk pelanggan atau karyawan.','addLabel'=>'Buat Survei','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Active','Closed'],'formFields'=>[['name'=>'title','label'=>'Judul Survei','type'=>'text','required'=>true],['name'=>'description','label'=>'Deskripsi','type'=>'textarea'],['name'=>'deadline','label'=>'Deadline','type'=>'date'],['name'=>'auth_required','label'=>'Login Diperlukan','type'=>'select','options'=>['Ya','Tidak']],['name'=>'scoring','label'=>'Mode Scoring','type'=>'select','options'=>['No Score','Score','Score with Answers']],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Active','Closed']]],'tableFields'=>[['name'=>'title','label'=>'Judul'],['name'=>'deadline','label'=>'Deadline'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/live-chat', fn() => view('erp.generic-module', ['title'=>'Live Chat','description'=>'Live chat dengan pelanggan di website langsung dari ERP, dengan chatbot AI.','features'=>['Chat Widget','Chatbot AI','Multi Operator','Chat History','Rating','Integrasi WhatsApp','Notifikasi Desktop']]));
Route::get('/erp/referral-program', fn() => view('erp.generic-module', ['title'=>'Referral Program','description'=>'Program referral pelanggan untuk mendatangkan pelanggan baru dengan reward.','features'=>['Referral Link','Reward Poin','Tracking Konversi','Dashboard','Expiry Date','Notifikasi']]));
Route::get('/erp/push-notification', fn() => view('erp.generic-module', ['title'=>'Push Notification','description'=>'Kirim notifikasi push ke pengguna mobile dan browser PWA.','features'=>['PWA Push','Mobile Push','Segmentasi','Schedule','Open Rate','Deep Link']]));

// ── WEBSITE & eCOMMERCE ────────────────────────────────────────────────
Route::get('/erp/website', fn() => view('erp.generic-module', ['title'=>'Website Builder','description'=>'Buat dan kelola website perusahaan dengan drag-and-drop editor.','features'=>['Drag & Drop Builder','Template Profesional','SEO Otomatis','Mobile Responsive','Blog','Form Builder','Analytics']]));
Route::get('/erp/ecommerce', fn() => view('erp.generic-module', ['title'=>'eCommerce','description'=>'Toko online terintegrasi penuh dengan inventory, pembayaran, dan pengiriman.','features'=>['Katalog Produk','Keranjang Belanja','Multiple Payment','Shipping Integration','Order Tracking','Product Reviews','Voucher Diskon']]));
Route::get('/erp/blog', fn() => view('erp.crud', ['module'=>'blogs','title'=>'Blog','description'=>'Kelola artikel dan konten blog perusahaan untuk SEO dan engagement.','addLabel'=>'Tulis Artikel','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Published','Archived'],'formFields'=>[['name'=>'title','label'=>'Judul','type'=>'text','required'=>true],['name'=>'author','label'=>'Penulis','type'=>'text'],['name'=>'category','label'=>'Kategori','type'=>'text'],['name'=>'content','label'=>'Konten','type'=>'textarea'],['name'=>'tags','label'=>'Tags','type'=>'text'],['name'=>'publish_date','label'=>'Tanggal Publish','type'=>'date'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Published','Archived']]],'tableFields'=>[['name'=>'title','label'=>'Judul'],['name'=>'author','label'=>'Penulis'],['name'=>'category','label'=>'Kategori'],['name'=>'publish_date','label'=>'Publish'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/forum', fn() => view('erp.generic-module', ['title'=>'Forum Diskusi','description'=>'Forum komunitas untuk pelanggan bertanya, berbagi pengalaman, dan berdiskusi.','features'=>['Thread & Reply','Voting','Tag/Label','Moderasi','Badge User','SEO Friendly','Notifikasi']]));
Route::get('/erp/elearning', fn() => view('erp.crud', ['module'=>'elearning-courses','title'=>'eLearning','description'=>'Platform kursus online untuk pelatihan karyawan atau pelanggan.','addLabel'=>'Buat Kursus','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Published','Closed'],'formFields'=>[['name'=>'title','label'=>'Judul Kursus','type'=>'text','required'=>true],['name'=>'instructor','label'=>'Instruktur','type'=>'text'],['name'=>'category','label'=>'Kategori','type'=>'text'],['name'=>'duration','label'=>'Durasi','type'=>'text'],['name'=>'price','label'=>'Harga','type'=>'number'],['name'=>'description','label'=>'Deskripsi','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Published','Closed']]],'tableFields'=>[['name'=>'title','label'=>'Kursus'],['name'=>'instructor','label'=>'Instruktur'],['name'=>'duration','label'=>'Durasi'],['name'=>'price','label'=>'Harga','format'=>'currency'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/seo', fn() => view('erp.generic-module', ['title'=>'SEO Manager','description'=>'Optimalkan halaman website untuk mesin pencari secara terpusat.','features'=>['Meta Tags','Sitemap Otomatis','Canonical URL','Schema Markup','Keyword Tracking','Google Search Console','PageSpeed Insights']]));

// ── SERVICES & PROJECT ─────────────────────────────────────────────────
Route::get('/erp/field-service', fn() => view('erp.crud', ['module'=>'field-services','title'=>'Field Service','description'=>'Kelola penugasan teknisi di lapangan, jadwal kunjungan, dan laporan servis.','addLabel'=>'Buat Penugasan','statusField'=>'status','filterField'=>'status','filterOptions'=>['New','In Progress','Completed','Cancelled'],'formFields'=>[['name'=>'task_title','label'=>'Judul Tugas','type'=>'text','required'=>true],['name'=>'customer','label'=>'Pelanggan','type'=>'text'],['name'=>'technician','label'=>'Teknisi','type'=>'text'],['name'=>'scheduled_date','label'=>'Jadwal','type'=>'date'],['name'=>'location','label'=>'Lokasi','type'=>'text'],['name'=>'description','label'=>'Deskripsi','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['New','In Progress','Completed','Cancelled']]],'tableFields'=>[['name'=>'task_title','label'=>'Tugas'],['name'=>'customer','label'=>'Pelanggan'],['name'=>'technician','label'=>'Teknisi'],['name'=>'scheduled_date','label'=>'Jadwal'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/helpdesk', fn() => view('erp.crud', ['module'=>'helpdesk-tickets','title'=>'Helpdesk — Tiket Support','description'=>'Sistem tiket dukungan pelanggan dengan SLA, prioritas, dan eskalasi.','addLabel'=>'Buat Tiket','statusField'=>'status','filterField'=>'status','filterOptions'=>['New','In Progress','Solved','Closed','Cancelled'],'formFields'=>[['name'=>'ticket_number','label'=>'No Tiket','type'=>'text'],['name'=>'subject','label'=>'Subjek','type'=>'text','required'=>true],['name'=>'customer','label'=>'Pelanggan','type'=>'text'],['name'=>'priority','label'=>'Prioritas','type'=>'select','options'=>['Low','Normal','High','Urgent']],['name'=>'category','label'=>'Kategori','type'=>'text'],['name'=>'assigned_to','label'=>'Ditugaskan ke','type'=>'text'],['name'=>'description','label'=>'Deskripsi','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['New','In Progress','Solved','Closed','Cancelled']]],'tableFields'=>[['name'=>'ticket_number','label'=>'No Tiket'],['name'=>'subject','label'=>'Subjek'],['name'=>'customer','label'=>'Pelanggan'],['name'=>'priority','label'=>'Prioritas','badge'=>true],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/planning', fn() => view('erp.crud', ['module'=>'planning-slots','title'=>'Planning — Perencanaan Shift','description'=>'Kelola jadwal shift karyawan dan alokasi sumber daya harian.','addLabel'=>'Tambah Jadwal','statusField'=>'status','filterField'=>'status','filterOptions'=>['Draft','Confirmed','Cancelled'],'formFields'=>[['name'=>'employee','label'=>'Karyawan','type'=>'text','required'=>true],['name'=>'role','label'=>'Peran','type'=>'text'],['name'=>'date','label'=>'Tanggal','type'=>'date'],['name'=>'start_time','label'=>'Mulai','type'=>'text'],['name'=>'end_time','label'=>'Selesai','type'=>'text'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Draft','Confirmed','Cancelled']]],'tableFields'=>[['name'=>'employee','label'=>'Karyawan'],['name'=>'role','label'=>'Peran'],['name'=>'date','label'=>'Tanggal'],['name'=>'start_time','label'=>'Jam Mulai'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/appointments', fn() => view('erp.crud', ['module'=>'appointments','title'=>'Appointments — Jadwal Janji','description'=>'Sistem booking jadwal janji temu online untuk pelanggan.','addLabel'=>'Buat Janji','statusField'=>'status','filterField'=>'status','filterOptions'=>['Booked','Confirmed','Done','Cancelled'],'formFields'=>[['name'=>'customer','label'=>'Pelanggan','type'=>'text','required'=>true],['name'=>'service','label'=>'Layanan','type'=>'text'],['name'=>'employee','label'=>'Staf','type'=>'text'],['name'=>'date','label'=>'Tanggal','type'=>'date'],['name'=>'time','label'=>'Jam','type'=>'text'],['name'=>'notes','label'=>'Catatan','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Booked','Confirmed','Done','Cancelled']]],'tableFields'=>[['name'=>'customer','label'=>'Pelanggan'],['name'=>'service','label'=>'Layanan'],['name'=>'employee','label'=>'Staf'],['name'=>'date','label'=>'Tanggal'],['name'=>'status','label'=>'Status','badge'=>true]]]));

// ── PRODUCTIVITY ───────────────────────────────────────────────────────
Route::get('/erp/discuss', fn() => view('erp.generic-module', ['title'=>'Discuss — Pesan & Diskusi','description'=>'Komunikasi internal tim melalui pesan langsung, grup, dan channel.','features'=>['Direct Message','Channel Grup','File Sharing','Mention','Thread','Notifikasi Real-time','Integrasi Modul']]));
Route::get('/erp/iot', fn() => view('erp.generic-module', ['title'=>'IoT — Internet of Things','description'=>'Hubungkan perangkat IoT ke ERP untuk otomasi dan monitoring real-time.','features'=>['Device Management','Sensor Data','Trigger Otomasi','Dashboard Monitor','Alert','Integrasi Manufaktur','MQTT Protocol']]));
Route::get('/erp/voip', fn() => view('erp.generic-module', ['title'=>'VoIP — Telepon IP','description'=>'Sistem telepon berbasis IP terintegrasi dengan CRM dan Helpdesk.','features'=>['Incoming Call Popup','Auto Log Call','Call Recording','Click to Call','Transfer','IVR','Laporan']]));
Route::get('/erp/notes', fn() => view('erp.crud', ['module'=>'notes','title'=>'Notes — Catatan','description'=>'Catatan pribadi dan tim yang bisa dibagikan ke kolega.','addLabel'=>'Buat Catatan','formFields'=>[['name'=>'title','label'=>'Judul','type'=>'text','required'=>true],['name'=>'content','label'=>'Isi Catatan','type'=>'textarea'],['name'=>'tags','label'=>'Tags','type'=>'text'],['name'=>'color','label'=>'Warna','type'=>'text']],'tableFields'=>[['name'=>'title','label'=>'Judul'],['name'=>'tags','label'=>'Tags']]]));
Route::get('/erp/calendar', fn() => view('erp.generic-module', ['title'=>'Kalender','description'=>'Kalender terpadu untuk event, meeting, deadline, dan jadwal karyawan.','features'=>['Google Calendar Sync','Meeting Room','Notifikasi','Recurring Event','Availabilty Check','Mobile Friendly','iCal Export']]));
Route::get('/erp/contacts', fn() => view('erp.crud', ['module'=>'contacts','title'=>'Kontak','description'=>'Buku alamat terpusat untuk semua pelanggan, supplier, dan mitra bisnis.','addLabel'=>'Tambah Kontak','formFields'=>[['name'=>'name','label'=>'Nama','type'=>'text','required'=>true],['name'=>'type','label'=>'Tipe','type'=>'select','options'=>['Pelanggan','Supplier','Karyawan','Mitra','Lainnya']],['name'=>'email','label'=>'Email','type'=>'email'],['name'=>'phone','label'=>'Telepon','type'=>'text'],['name'=>'mobile','label'=>'HP','type'=>'text'],['name'=>'company','label'=>'Perusahaan','type'=>'text'],['name'=>'address','label'=>'Alamat','type'=>'textarea'],['name'=>'website','label'=>'Website','type'=>'text']],'tableFields'=>[['name'=>'name','label'=>'Nama'],['name'=>'type','label'=>'Tipe','badge'=>true],['name'=>'email','label'=>'Email'],['name'=>'phone','label'=>'Telepon'],['name'=>'company','label'=>'Perusahaan']]]));

// ── CRM PIPELINE ───────────────────────────────────────────────────────
Route::get('/erp/crm', fn() => view('erp.generic-module', ['title'=>'CRM — Pipeline Penjualan','description'=>'Kelola pipeline prospek dan peluang penjualan dalam tampilan Kanban.','features'=>['Kanban Pipeline','Lead Scoring','Activity Planning','Email Integration','Sales Forecasting','Lost Analysis','Team Dashboard']]));
Route::get('/erp/crm-leads', fn() => view('erp.crud', ['module'=>'crm-leads','title'=>'CRM Leads','description'=>'Daftar prospek (leads) yang masuk dari berbagai sumber.','addLabel'=>'Tambah Lead','statusField'=>'status','filterField'=>'status','filterOptions'=>['New','Qualified','Converted','Lost'],'formFields'=>[['name'=>'lead_name','label'=>'Nama Lead','type'=>'text','required'=>true],['name'=>'contact_name','label'=>'Nama Kontak','type'=>'text'],['name'=>'email','label'=>'Email','type'=>'email'],['name'=>'phone','label'=>'Telepon','type'=>'text'],['name'=>'source','label'=>'Sumber','type'=>'select','options'=>['Website','Referral','Cold Call','Social Media','Event','Lainnya']],['name'=>'expected_revenue','label'=>'Estimasi Nilai','type'=>'number'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['New','Qualified','Converted','Lost']]],'tableFields'=>[['name'=>'lead_name','label'=>'Lead'],['name'=>'contact_name','label'=>'Kontak'],['name'=>'source','label'=>'Sumber'],['name'=>'expected_revenue','label'=>'Est. Nilai','format'=>'currency'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/crm-opportunities', fn() => view('erp.crud', ['module'=>'crm-opportunities','title'=>'Peluang (Opportunities)','description'=>'Kelola peluang penjualan yang sedang dalam proses negosiasi.','addLabel'=>'Tambah Peluang','statusField'=>'stage','filterField'=>'stage','filterOptions'=>['Qualification','Proposal','Negotiation','Won','Lost'],'formFields'=>[['name'=>'name','label'=>'Nama Peluang','type'=>'text','required'=>true],['name'=>'customer','label'=>'Pelanggan','type'=>'text'],['name'=>'salesperson','label'=>'Sales','type'=>'text'],['name'=>'expected_revenue','label'=>'Est. Pendapatan','type'=>'number'],['name'=>'probability','label'=>'Probabilitas (%)','type'=>'number'],['name'=>'close_date','label'=>'Target Tutup','type'=>'date'],['name'=>'stage','label'=>'Tahap','type'=>'select','options'=>['Qualification','Proposal','Negotiation','Won','Lost']]],'tableFields'=>[['name'=>'name','label'=>'Peluang'],['name'=>'customer','label'=>'Pelanggan'],['name'=>'expected_revenue','label'=>'Est. Nilai','format'=>'currency'],['name'=>'probability','label'=>'Prob.'],['name'=>'stage','label'=>'Tahap','badge'=>true]]]));
Route::get('/erp/crm-activities', fn() => view('erp.crud', ['module'=>'crm-activities','title'=>'Aktivitas CRM','description'=>'Log semua aktivitas sales: panggilan, email, meeting, demo, dan follow-up.','addLabel'=>'Tambah Aktivitas','statusField'=>'status','filterField'=>'status','filterOptions'=>['Planned','Done','Overdue','Cancelled'],'formFields'=>[['name'=>'activity_type','label'=>'Tipe','type'=>'select','options'=>['Telepon','Email','Meeting','Demo','Follow Up','Lainnya']],['name'=>'customer','label'=>'Pelanggan','type'=>'text'],['name'=>'salesperson','label'=>'Sales','type'=>'text'],['name'=>'date','label'=>'Tanggal','type'=>'date'],['name'=>'note','label'=>'Catatan','type'=>'textarea'],['name'=>'status','label'=>'Status','type'=>'select','options'=>['Planned','Done','Overdue','Cancelled']]],'tableFields'=>[['name'=>'activity_type','label'=>'Tipe'],['name'=>'customer','label'=>'Pelanggan'],['name'=>'salesperson','label'=>'Sales'],['name'=>'date','label'=>'Tanggal'],['name'=>'status','label'=>'Status','badge'=>true]]]));
Route::get('/erp/customer-segments', fn() => view('erp.crud', ['module'=>'customer-segments','title'=>'Segmentasi Pelanggan','description'=>'Kelompokkan pelanggan berdasarkan kriteria untuk marketing yang lebih tepat sasaran.','addLabel'=>'Buat Segmen','formFields'=>[['name'=>'segment_name','label'=>'Nama Segmen','type'=>'text','required'=>true],['name'=>'criteria','label'=>'Kriteria','type'=>'textarea'],['name'=>'member_count','label'=>'Jumlah Member','type'=>'number'],['name'=>'notes','label'=>'Catatan','type'=>'textarea']],'tableFields'=>[['name'=>'segment_name','label'=>'Segmen'],['name'=>'criteria','label'=>'Kriteria'],['name'=>'member_count','label'=>'Member']]]));

// ── DELIVERY TAMBAHAN ──────────────────────────────────────────────────
Route::get('/erp/driver-areas', fn() => view('erp.generic-module', ['title'=>'Wilayah Driver','description'=>'Kelola pembagian wilayah pengiriman per driver.','features'=>['Mapping Wilayah','Assign Driver','Radius Pengiriman','Peta Interaktif','Optimasi Rute','Laporan']]));
Route::get('/erp/delivery-routes', fn() => view('erp.generic-module', ['title'=>'Rute Pengiriman','description'=>'Optimasi rute pengiriman untuk efisiensi biaya dan waktu.','features'=>['Route Optimization','Multi-Stop','Distance Calculation','ETA','Live Tracking','Laporan Rute']]));
Route::get('/erp/shipping-cost', fn() => view('erp.generic-module', ['title'=>'Biaya Pengiriman','description'=>'Kelola tarif ongkos kirim berdasarkan berat, jarak, dan zona.','features'=>['Tarif per Zona','Berat & Dimensi','Ekspedisi Integration','Free Shipping Rule','COD Handling','Laporan']]));

// ── AI & AUTOMATION ────────────────────────────────────────────────────
Route::get('/erp/ai-demand', fn() => view('erp.generic-module', ['title'=>'AI Demand Planning','description'=>'Prediksi permintaan produk menggunakan machine learning untuk optimasi stok.','features'=>['Demand Forecast','Seasonal Adjustment','Auto Reorder','Confidence Score','Historical Analysis','Safety Stock Calc']]));
Route::get('/erp/smart-pricing', fn() => view('erp.generic-module', ['title'=>'Smart Pricing','description'=>'Dynamic pricing berbasis AI berdasarkan kompetitor, permintaan, dan margin.','features'=>['Dynamic Price','Competitor Monitoring','Margin Protection','Price Rule','Customer Segment Pricing','Approval Workflow']]));
Route::get('/erp/ocr', fn() => view('erp.generic-module', ['title'=>'OCR — Scan Dokumen','description'=>'Scan dan ekstrak data dari dokumen fisik secara otomatis menggunakan AI.','features'=>['Invoice OCR','Receipt Scan','Auto Fill Form','Multi Language','PDF Extract','Approval Workflow']]));

// ── MULTI BRANCH ───────────────────────────────────────────────────────
Route::get('/erp/multi-branch', fn() => view('erp.generic-module', ['title'=>'Multi Branch Analytics','description'=>'Dashboard perbandingan kinerja antar cabang secara real-time.','features'=>['Perbandingan Cabang','Sales per Cabang','Inventory per Cabang','P&L per Cabang','Transfer Antar Cabang','Laporan Konsolidasi']]));
Route::get('/erp/multi-branch-setup', fn() => view('erp.generic-module', ['title'=>'Multi Branch Setup','description'=>'Konfigurasi cabang, gudang, dan akses user per lokasi.','features'=>['Tambah Cabang','Assign User','Gudang per Cabang','Prefix Nomor Dokumen','Inter-company Transaction','Konsolidasi']]));

// ── SYSTEM ─────────────────────────────────────────────────────────────
Route::get('/erp/theme', fn() => view('erp.generic-module', ['title'=>'Tema & Tampilan','description'=>'Kustomisasi tampilan ERP: warna, logo, dan preferensi antarmuka.','features'=>['Ganti Warna','Upload Logo','Dark Mode','Font Size','Sidebar Style','Custom CSS']]));
Route::get('/erp/backup-system', fn() => view('erp.generic-module', ['title'=>'Backup Sistem','description'=>'Backup otomatis database dan file dengan penjadwalan dan restore point.','features'=>['Auto Backup','Manual Backup','Cloud Storage','Restore','Enkripsi','Notifikasi']]));
Route::get('/erp/api-public', fn() => view('erp.generic-module', ['title'=>'API Public','description'=>'Dokumentasi dan manajemen API key untuk integrasi dengan sistem eksternal.','features'=>['API Key Management','Swagger Docs','Rate Limiting','Webhook','OAuth2','Log Request']]));
Route::get('/erp/sync', fn() => view('erp.generic-module', ['title'=>'Sinkronisasi','description'=>'Sinkronisasi data antar sistem: Kledo, marketplace, dan aplikasi lainnya.','features'=>['Sync Kledo','Sync Marketplace','Sync Stok','Sync Pelanggan','Schedule Sync','Log Sync']]));
Route::get('/erp/mobile-sync', fn() => view('erp.generic-module', ['title'=>'Mobile Sync','description'=>'Sinkronisasi data ke aplikasi mobile driver dan sales.','features'=>['Driver App Sync','Sales App Sync','Offline Mode','Push Data','Pull Data','Conflict Resolution']]));
Route::get('/erp/payment-gateway', fn() => view('erp.generic-module', ['title'=>'Payment Gateway','description'=>'Integrasi payment gateway untuk pembayaran online dari pelanggan.','features'=>['Midtrans','Xendit','Stripe','QRIS','Virtual Account','COD','Pembayaran Cicilan']]));
